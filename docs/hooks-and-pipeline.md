...
| `external_review_state.json` | /shipwright-plan Step 5b, /shipwright-iterate (medium+) — via `mark-review-state.py`, carrying per-reviewer `verdicts` + derived `contradiction`. Current writers emit `marker_schema: 4` with `glm`/`openai`; readers also accept historical schema-3 `deepseek`/`openai` markers, historical schema-2 `gemini`/`openai` markers (and older markers without a schema) and fail closed on mixed/triple reviewer sets. The external-review CLI likewise emits `review_schema: 2`; its historical schema 1 was implicit and used `gemini`/`openai`. | /shipwright-plan Step 6 gate (`check-plan-gates.py --gate review`), `setup-planning-session.py` resume gate, compliance `W5`, evidence collector — all three via `evaluate_review_state` |
