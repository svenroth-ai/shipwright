...
| `.shipwright/runtime/events-context-index.json` | `shared/scripts/tools/event_context.py` | the same tool; disposable, gitignored, fingerprint-validated and fully rebuildable from the unchanged raw event log + area catalog. Never an audit source. |
