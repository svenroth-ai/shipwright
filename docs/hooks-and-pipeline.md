...
**Honesty artifacts (Step E + Step E.18, `trg-1aa5a8ab`).** An onboarded project
is not required to arrive perfect, only to arrive honestly described. Two
artifacts carry that, and `validate_adoption.py` hard-errors when either is
missing — so Step H stops rather than handing over a catalogue that reads as
confirmed:

| Artifact | Written by | Says |
|---|---|---|
| `.shipwright/adopt/derived-catalogue.json` | Step E, `scripts/lib/derived_catalogue.py` | one row per derived requirement (`fr_id`, `name`, `basis`, `confirmed`) plus `total` / `unconfirmed` / `by_basis`. The rendered `spec.md` carries the same facts as a prose block above the FR table; both come from ONE `spec_table.effective_features` pass, so the count cannot describe a different table than the one handed over. |
| `shipwright_known_failures.json` | Step E.18, `scripts/tools/record_inherited_baseline.py` | `known_failures[]` + `baseline_failure_count` in the shape `shipwright-compliance`'s `collect_known_failures` already reads (until now a consumer contract with **no producer**), plus an additive `inherited_coverage_gaps` block. A coverage gap NEVER feeds `baseline_failure_count` — that number excuses a `passed < total` gap in `rtm_generator`, and a missing test must not buy forgiveness for a failing one. `baseline_observed: false` records "no run was made", never a confident zero. |

Step E.18 is also the sole triage-filing owner here (it is the first step after
E.16 scaffolds the Inbox): one idempotent card asking that the derived catalogue
be questioned with a person per `shared/requirement-elicitation.md`
...
journey-coverage gap routes to instead of blocking a test run. Teaching the
**test** phase to read the register is the other half of the split and is
tracked separately (`trg-12b4cf3f`).
...
| `.shipwright/adopt/derived-catalogue.json` | adopt Step E (`scripts/lib/derived_catalogue.py`) | adopt re-runs (idempotent overwrite). Records which requirements were DERIVED from reading code and how many nobody has confirmed, so traceability / coverage / drift consumers can tell an unconfirmed catalogue from a confirmed one without parsing prose. Read by adopt Step E.18 and by the Step H handover (`unconfirmed` → the commit body's required `unconfirmed_fr_count` + the banner). `trg-1aa5a8ab` |
| `shipwright_known_failures.json` | adopt Step E.18 (`scripts/tools/record_inherited_baseline.py`) | adopt re-runs (idempotent overwrite); an operator may hand-edit. **Read by** `plugins/shipwright-compliance/scripts/lib/collectors/test_evidence.collect_known_failures` — which existed as a consumer contract with no producer until `trg-1aa5a8ab`, so every inherited red test read as this project's own failure. Two blocks, deliberately separate: `known_failures[]` + `baseline_failure_count` (tests already failing before onboarding — this number turns a `passed < total` gap into `COVERED (baseline)` in `rtm_generator`, i.e. it buys forgiveness) and `inherited_coverage_gaps` (requirements with no `@FR`-tagged test, plus tests switched off) which **never** contributes to it. `baseline_observed: false` / `baseline_source: "not_run"` is the honest default — onboarding does not run an arbitrary repo's suite. The test phase learning to read it is `trg-12b4cf3f`. |
