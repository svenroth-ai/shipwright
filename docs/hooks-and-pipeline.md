...
| `.shipwright/agent_docs/decision-drops/INDEX.md` | `shared/scripts/lib/decision_drops_index.py` | Local-only convenience index of pending (not yet folded) decision-drops, refreshed by `write_decision_drop.py` (a new drop was added) and `aggregate_decisions.py` (drops were folded and deleted). **Deliberately gitignored, unlike the ADR/decision-log indexes** — the decision-drops directory itself is gitignored (main-repo path, per `shared/glossary.md`), so this index is never committed, carries no `CHURN_ALLOWLIST` entry (git can never see a conflict on an ignored path), and has no CI drift guard against a committed copy. Concurrent local writers (parallel iterate worktrees all resolve the SAME shared main-repo directory) are guarded by a file lock instead of git merge semantics. Refresh by hand with `uv run {shared_root}/scripts/tools/rebuild_decision_drops_index.py --project-root .`. |
