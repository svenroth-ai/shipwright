...
|----------|-----------|-----------|
| `CLAUDE.md` | project | — |
| `CONTEXT.md` (target project root, domain glossary) | `/shipwright-project` interview (`shared/scripts/tools/write_context_term.py`, one `Language` entry per sharpened term, `file_lock` + `durable_atomic_write`) | the same producer on every re-sharpen (idempotent upsert; `Relationships` / `Flagged ambiguities` preserved but never written). Format SSoT: `shared/context-format.md`. **Nothing in the pipeline currently reads it back** — it is a human-facing glossary only. |
