...
| `.shipwright/agent_docs/decision_log_index.md` | **regenerate** (same treatment as the ADR INDEX row above, for the same reason — `lib.decision_log_index.rebuild_decision_log_index` re-derives it as a pure function of `decision_log.md`'s own already-merged content). Refreshed by TWO producers rather than one: `write_decision_log.py` (the direct-append path plan/build/deploy use) and `aggregate_decisions.py` (the release-time fold), because either can be the last write to `decision_log.md` before a commit. Also refreshed post-merge by `integrate_regenerate.regenerate_after_merge` alongside the ADR index. Not a `DERIVED_SNAPSHOTS` member for the same reason the ADR index isn't. **Deliberately no sibling row for `decision-drops/INDEX.md`** — that ONE file (not the directory it lists, which is tracked since iterate-2026-08-08-track-decision-drops) stays gitignored, so git never sees a conflict on it and an allowlist entry would be exercised by nothing; a concurrent write within the SAME checkout is guarded by a file lock instead. **Also no row for `decision-drops/*.json` itself** — each drop is a uniquely-named new file per run, so two branches adding different ones merges cleanly with no conflict to resolve (the same shape `CHANGELOG-unreleased.d/` already relies on); it needs neither `merge=union` nor a `CHURN_ALLOWLIST` entry. Refresh the index by hand with `uv run {shared_root}/scripts/tools/rebuild_decision_log_index.py --project-root .`. |
...
(`--stage` · `--pr` · `--restore` · `--verify-commit` [`--release-audit`
modifier] · `--stamp-adopted`), whose
...
* **refuses** (`skipped`, mutating nothing) when main's state is not understood:
  ...
  `main_tracked_glued_line` (a producer event recognisably glued to other
  content on one physical line — narrower than `main_tracked_unparseable`,
  both name `triage_repair.py`; iterate-2026-08-07-triage-adopt-glued-refusal),
...
- **F11 single iterate** (incl. independently-concurrent iterates) runs
  ...
  **Before that merge, `ensure_current` also absorbs a dirty tracked
  ...
  (`_absorb_dirty_triage_log`, P2.43). The compliance-backlog Stop hook
  refreshes this file on every Stop for an iterate run's whole duration, so a
  finalization spanning more than one Stop can dirty it again after F6's own
  commit — exactly the window this guard runs in — and `git merge` refuses
  outright ("local changes ... would be overwritten") the moment mainline
  also touched the path. Unlike `stash_run_written`'s two members above,
  `triage.jsonl` is committed rather than carried in memory: it is an
  append-only log with its own churn-merge resolver
  (`churn_merge.CHURN_ALLOWLIST`), so folding it into a real commit lets that
  resolver reconcile a genuine divergence instead of one side silently
  winning.
...
A plugin may also ship a sibling `hooks-codex/hooks.json` (same shape) for a
hook that only the Codex bundle build should register — see "Runtime-scoped
hook registration (R2)" below. Claude Code's own loader never reads it.
...
### Compliance backlog lifecycle authority

Compliance detection and backlog mutation are deliberately separate. The Stop hook is **branch feedback**: it runs A-I against the resolved branch tree and reports failures locally, but never appends, refreshes, or dismisses the global compliance backlog. After `deliver_pr.py` reports `DELIVERED`, delivery re-reads the delivered PR and audits its exact `mergeCommit` in a detached tree; Groups A-D and F-I may converge the backlog there. Group E is explicitly `not_applicable` at merge — not missing coverage — because its seven evidence documents are release-owned. A missing/crashed/import-gated expected group is instead `missing` and leaves the backlog untouched. After Step 6 has committed the staged evidence (Step 5.5 only stages it) and `refresh_compliance_docs.py --verify-commit --release-audit` succeeds, release runs the authoritative full A-I audit and may converge Group E too. The `--release-audit` flag is what gates this — `--verify-commit` alone (as adopt Step H calls it, `references/step-h-validate-commit-handoff.md`) stays a pure evidence verifier and never spawns the lifecycle audit or touches the global backlog. `audit_compliance_lifecycle.py --scope branch_feedback` is a manual CLI equivalent of the Stop hook's own in-process branch feedback — an operator's on-demand check, not a producer any hook or script calls; it never converges the backlog either.

**Known residual: coverage is group-granular, dismissal is check-granular.** `Coverage` tracks whether a *group* ran; a group that ran but had one check inside it degrade to `skip` for an environment reason (e.g. group A5's PyYAML-unavailable path, `group_a5.py`) still counts as fully covered, so that check's findings silently drop out of the merge/release audit's fails-set — a backlog card citing it can be dismissed as resolved when the check simply never ran. Bounded, not open-ended: the three PyYAML-dependent audit invocation sites (both Stop hooks, in-process; plus `audit_compliance_lifecycle.py`'s merge and release spawns) all carry the specific dependency (`uv run --with pyyaml`) the one known instance of this needs, and any recurrence self-heals — a later audit that DOES see the finding reopens the dismissed card via the `complianceRegressed` path (`triage_bundle.py`), so the failure mode is losing one convergence cycle, not the finding.

**Known residual: an amended preserve-group card's `launchPayload` can go stale.** `amend_triage_item` can change `title`/`detail`/`severity`/`kind` only — `dedupKey` and `launchPayload` are frozen at first append (AC-8) and never rewritten on a merge amend, so the launch fence an operator copies to start a fix session can list a stale, shorter finding list than the amended `detail` above it — a starting point, not a live sync. Bounded by the next release rotating the card, which rebuilds `launchPayload` from scratch via `build_launch_payload` over the true current fails-set. (Severity escalation is computed from the newly-observed fails only — a fold-back's fabricated placeholder severity for a preserved-but-not-currently-failing group can no longer inflate a card past its real severity, doubt review round 5.)

**Known residuals, deferred as bounded follow-ups (doubt review round 5):** two narrow gaps left intentionally open rather than expanding this change's scope further. (1) Merge convergence has no ordering guard against two deliveries landing inside the same audit window — whichever finishes last wins the write, so a stale re-append of an already-fixed finding is possible but self-heals on the next delivery. (2) The worktree-reclaim sweep's temp-dir-location guard (`_is_own_detached_worktree`) binds to the *sweeping* process's own `tempfile.gettempdir()` — if the orphaning and sweeping runs see genuinely different temp roots, the orphan's registration is never reclaimed by name/location alone. Neither corrupts the backlog or destroys live data; both are candidates for a follow-up (a merge-ordering check via `git merge-base --is-ancestor`, and a creation-time marker file inside the worktree instead of/alongside path ancestry).

### Shared Hook: audit_compliance_on_stop.py

**Script:** `shared/scripts/hooks/audit_compliance_on_stop.py` —
Stop-event trigger for **compliance branch feedback**. Added
iterate-2026-05-30; authority moved to `audit_compliance_lifecycle.py`
2026-08-09 (P2.59, branch-feedback authority). Wired into the
...
**History:** until P2.59 this hook called `audit_detector.mirror_findings_to_triage`
directly on every Stop, making it the automatic trigger for
`source=compliance` triage items (Option A1, 2026-05-23 design
discussion — every *other* triage producer had a frequent automatic
trigger, and this was the lone exception). That authority now belongs to
`audit_compliance_lifecycle.py`; see **Authority** below. This hook still
does NOT touch the #78 snapshot-provenance E-staleness machinery.

**Contract:** non-blocking and idempotent per `(HEAD-sha, session_id)`, with a runtime marker; no-op off managed projects and when `SHIPWRIGHT_COMPLIANCE_AUDIT_ON_STOP=0`.

**Authority:** Stop is branch feedback only. It resolves the active worktree from the run pointer, detects full A-I, and prints local failures (including expected Group-E pending-release drift). It never calls the triage mirror or changes `.shipwright/triage.jsonl`. The lifecycle runner performs global convergence only after delivered merge authority (A-D/F-I, E `not_applicable`) or verified release authority (A-I).
...
| SubagentStop | `shipwright-build:spec-reviewer` | `write-review-payload-on-stop.py --review-type spec` | **Review-findings durability backstop** (iterate-2026-08-09-compaction-state-audit). Step 8's own mandate is "write the reviewer's reply to `reviews.json` before any other reasoning" — a compaction landing in that window loses the finding outright since the orchestrator only holds it in a Task-tool result, never a file. Same salvage pattern as `write-section-on-stop.py` (self-contained, no `lib.*` import — a second `lib` package exists at `plugins/shipwright-build/scripts/lib`, so an import would hit the ADR-044 same-name collision): reads the subagent's own transcript, extracts a `iterate-YYYY-MM-DD-...` run_id and the last assistant reply, and — only if `reviews.json` doesn't already have a terminal record for this review type — calls `record_review_pass.py` to salvage it. NEVER blocks (bad stdin, missing transcript, no run_id, non-JSON reply all no-op). |
| SubagentStop | `shipwright-build:code-reviewer` | `write-review-payload-on-stop.py --review-type code` | Same backstop, for the code-reviewer pass. |
| SubagentStop | `shipwright-build:code-reviewer` | `cleanup-review-scratch-on-code-reviewer-failure.py` | **Review-scratch failure-path safety net** (PR #676 external-review finding, `iterate-2026-09-03-review-scratch-path`). `code-review-protocol.md` Step 6b writes the diff via `review_scratch.py resolve` and deliberately leaves cleanup to Step 6c (the external cascade), which reuses the same file — but if the code-reviewer subagent crashes or returns no parseable review, 6c is never reached the intended way and the diff would otherwise linger. Reads the subagent's own transcript (same salvage pattern as the sibling hook above, self-contained for the same ADR-044 reason); if the last reply is NOT a parseable review payload, calls `review_scratch.py cleanup --run-id "$SHIPWRIGHT_SESSION_ID"`. A no-op on a successful reply (6c still owns cleanup then) and a no-op if 6c already cleaned up (`cleanup()` is idempotent). NEVER blocks. |
| SubagentStop | `shipwright-build:doubt-reviewer` | `write-review-payload-on-stop.py --review-type doubt` | Same backstop, for the doubt-reviewer pass. |
...
| PostToolUse | `Write\|Edit` | `check_file_size.py` | Non-blocking nudge + per-session marker writer. Crossings of the 300/400 line guideline (300 source/test, 400 runtime-prompt SKILL.md/CLAUDE.md/agents) emit a stdout nudge AND write `<repo-root>/.shipwright/locks/bloat_pending.<session_id>.json` (atomic tmp+rename). The marker/baseline/re-measure root is resolved via `repo_root.main_repo_root_or(Path.cwd())` (fail-soft adapter over `worktree_isolation.main_repo_root`), **never `Path.cwd()`** — a PostToolUse firing with cwd≠repo-root (sub-package test run, monorepo auto-descent) would otherwise leak the marker to a nested `shared/.shipwright/locks/` the root-anchored gitignore misses (fixed `iterate-2026-06-09-idle-main-artifact-hygiene`; a non-anchored `**/.shipwright/locks/` canon ignore is belt-and-suspenders). The Stop-Gate (`bloat_gate_on_stop.py`) resolves the SAME root + reads that marker. Registered in every plugin's `hooks.json` since Campaign A.foundation. `<session_id>` in the filename is actually a composite `bloat_baseline.marker_key(payload, env)` value: the hook **stdin payload**'s `session_id`, falling back to the `SHIPWRIGHT_SESSION_ID` env var then `"unknown"` — both writer and gate must agree (fixed `iterate-2026-05-29-bloat-gate-session-id`; env-only keying pooled every session into one `unknown` bucket so one session's oversize file blocked another's Stop) — **plus a `.<agent_id>` suffix** when the payload carries `agent_id` (Claude Code sets it only when the hook fires inside a subagent call, e.g. a background `sub-iterate-runner` Task). A subagent shares its spawning session's `session_id`, so without the suffix its still in-flight, uncommitted marker pooled into the orchestrator's own — blocking the orchestrator's Stop on a file it never touched and had no way to fix (fixed `2026-09-07`, campaign-req3-06-mechanics-webui). The suffix logic lives in `bloat_marker_key.py` (split out of `bloat_baseline.py` to stay under its own 300-line ceiling; the encoding is a collision-free `~XX` hex-escape, not lossy character substitution — external review, 2026-09-07); `bloat_baseline.marker_key` re-exports it. `bloat_gate_on_stop.py` is registered on `Stop` only, never `SubagentStop`, so its own invocation never actually carries `agent_id` in production — a subagent's own oversize edit is not interactively gated by THIS hook (its own F0/F11 finalization is the real backstop); the suffix exists solely to stop a sibling subagent's in-flight marker from blocking the spawning session's own Stop. The Stop-Gate clears an **anti-ratchet** entry when the file is trimmed back to `<=` its baseline `current` (the grandfathered ceiling) — it blocks only when the live size grew PAST `current`, matching the canonical anti-ratchet rule in `anti_ratchet.py` (same iterate; previously it compared only against the 300 limit, so a correctly-trimmed grandfathered file kept blocking). |
| PostToolUse | — (catch-all) | `track_tool_calls.py` | Increments tool call counter for context pressure detection |
| Stop | — | `bloat_gate_on_stop.py` | Blocks completion when bloat markers indicate anti-ratchet or a new crossing outside the baseline allowlist (`shipwright_bloat_baseline.json`). Session-scoped (reads only the current session's marker via `bloat_baseline.marker_key` — see the `check_file_size.py` row above for the `agent_id`-suffix isolation story — falls back to `unknown` when `SHIPWRIGHT_SESSION_ID` is unset). Registered on `Stop` only, never `SubagentStop`, so this hook's own invocation never actually carries `agent_id` in production — it enforces the SPAWNING session's own edits only; a subagent's own oversize edit is caught (if at all) by its own F0/F11 finalization's independent `bloat_baseline.scan()`, not by this hook (internal plan review, 2026-09-07). Re-measures each entry at decision time so a fixed file isn't punished. Pass-silently when no baseline file exists (fresh / pre-adopt repos). Registered in every plugin's `hooks.json` since Campaign A.foundation. |
...
| Stop | — | `track_context_cost.py` (shared) | **Context-cost meter (context-cost-meter, additive — `track_tool_calls.py` above is left running unmodified; cutover is a separate follow-up iterate).** Recomputes the session's ENTIRE transcript on every firing (dedup by `requestId`, phase-tagged via the existing `iterate_phase_groups` boundary-mark sidecar, priced per-token-type incl. the cache-read/cache-write TTL split — never blended into the input rate) and overwrites one JSON file, `.shipwright/compliance/context-cost/<session_id>.json`. No incremental cache: sessions are small (median ~276 calls) and cheap to re-read in full, and full-recompute-and-overwrite eliminates concurrent-`Stop`-race and stale-cache classes by construction (both external review rounds on this feature converged on this over a standing append-only cache). Session id is resolved stdin payload FIRST, `SHIPWRIGHT_SESSION_ID` env var only as fallback (`context_cost_core.resolve_session_id`, shared with `context_cost_statusline.py`) — the SAME rule the `check_file_size.py` row above documents for `bloat_gate_on_stop.py`: a subprocess Claude Code spawns directly for `Stop`/`statusLine.command` does not reliably inherit that env var, so the payload is the only channel this process class can trust. `estimate_context_pressure.py --source context-cost` (opt-in, default stays `toolcall`) and `finalize_iterate.py`'s F5b fold are a DIFFERENT process class — plain Bash-tool-invoked scripts with no stdin payload at all — and read the env var directly, which they can trust because the assistant's own Bash-tool children reliably inherit it via `CLAUDE_ENV_FILE` (`capture_session_id.py`). An unrecognized model id never falls back to another model's price — `model_pricing.compute_cost_usd` returns `(None, True)` for that call, which the aggregate rolls into `unpriced_calls` (never into `cost_usd`) and flags via `cost_complete: false`; the persisted summary has no per-call rows, but its `unpriced_models` field (the sorted, deduplicated list of model ids that produced an unpriced call) tells a caller WHICH model needs a pricing-table entry without one. Never blocks shutdown: every failure is caught, logged to stderr, and the hook still returns 0 (`generate_handoff_on_stop.py`'s own contract). Prints nothing to stdout (Stop hooks cannot carry `hookSpecificOutput`). Folded into the run's tracked `work_completed` event at F5b (`context_cost_core.fold_into_event`, additive/first-write-wins like `phase_timings`), stamped `measured_through`/`measured_at` so that F5b-bounded total (it cannot include F6-F12) is never mistaken for a whole-session figure. `context_cost_statusline.py` (operator-wired, not auto-registered — a plugin cannot write a user's personal `statusLine.command` any more than it can set `autoCompactWindow`) and `context_cost_readiness.py` (autoCompactWindow-vs-context-window + effort-level advisory, same report shape as `verify_local.py`) read the same file/settings but are not themselves hooks. |
...
| UserPromptSubmit | — | `codex_activation_mint.py` | **Codex activation mint (R2/M3, AC1a).** No-ops unless `is_codex_runtime()` (`lib.codex_runtime`) — a documented no-op under Claude Code. Parses the incoming prompt via `lib.codex_envelope_grammar.parse()`; mints an activation record (`lib.codex_activation_record.mint()`, exclusive-create-only) recording `armed: true` (grammar matched) or `armed: false`. Because mint is exclusive-create, only the session's FIRST `UserPromptSubmit` actually decides the armed state — every later prompt finds the record already exists and this hook does nothing further, by construction, not by an extra check of its own. Writes nothing to `additionalContext` — a silent side-effect producer for the `PreToolUse` gate below to read. Threat model: cooperative local-process enforcement, not an adversarial boundary — a model/process with shell access in the same workspace could write its own armed/`.consumed` state; the guarantee is "a cooperating-but-forgetful session can't silently skip the worktree-setup step" (external review, openai low-7), consistent with R0's envelope-authenticity framing. |
| PreToolUse | — (catch-all) | `codex_pretooluse_gate.py` | **Codex first-call gate (R2/M3, AC1a).** No-ops unless `is_codex_runtime()`. Reads the session's activation record (`lib.codex_activation_record.read()`); absent or `armed: false` → allow, always (fail-open, R0's Contract verbatim — this path must never deny). `armed: true` and the session's own durable first-call-settled flag (a separate exclusive-consume `.consumed` marker) still unset → the call is evaluated: a `Bash`-class call is tokenized (`shlex`) and matched against `setup_iterate_worktree.py` by basename, never substring — single-segment match, or a sequential (`&&`/`;`-only) chain whose every prefix segment is a bare `cd <path>` and whose last segment matches; any `\|`/`\|\|` in the command denies outright regardless of match position (closes a compound-command-smuggling gap a code review found: `rm -rf x && uv run setup_iterate_worktree.py` previously allowed in full because one segment matched). Any non-`Bash`/unrecognized tool class denies by default. A matching call consumes the settle-flag and allows; anything else consumes and denies (`hookSpecificOutput.permissionDecision: "deny"`, reason includes the exact runnable setup command). Once settled, every later call in the session allows unconditionally — the gate only ever evaluates the session's first eligible call. Scope: local function-tool path only — Codex's hosted tools bypass `PreToolUse` entirely, a Codex-side fact this hook cannot change. Basename-only matching is an accepted residual risk (a same-named script elsewhere on disk would also satisfy it) under the same cooperative-enforcement threat model as the mint hook above, not a hardened adversarial boundary. |
...
| Stop | — | `audit_compliance_on_stop.py` (shared) | **Compliance branch feedback.** Runs local A-I diagnostics only; global backlog convergence is reserved for delivered merge and verified release authority. Ordered after phase_quality; idempotent and non-blocking. |
| Stop | — | `generate_handoff_on_stop.py` (shared) | Session handoff |
...
Writes: `CLAUDE.md`, `.shipwright/agent_docs/{architecture,conventions,decision_log,build_dashboard}.md`, `.shipwright/planning/<split>/spec.md`, `.shipwright/planning/adr/<NNN>-<slug>.md` + `.shipwright/planning/adr/INDEX.md` (adopt's own minted ADRs — the adoption ADR plus any retroactive ADRs, mirrored into decision_log.md's inline copy; see below), all six `shipwright_*_config.json` (run-config LAST), `shipwright_events.jsonl` (one `adopted` event + optional backfill), `e2e/flows/adopted-baseline.spec.ts` when a Playwright crawl succeeded, `.shipwright/adopt/{snapshot,enrichment,routes}.json`, `.shipwright/adopt/derived-catalogue.json` + `shipwright_known_failures.json` (the two **honesty artifacts** — see below), `.shipwright/adopt/review.md`, four `.github/workflows/*.yml` files (security, profile-specific CI, profile-aware CodeQL, claude-review — each idempotent and never overwritten), the repo-root `AUTOMERGE_SETUP.md` branch-protection / auto-merge guide (written LAST so its Required-Check job-name list is derived from the deployed workflow files), and seeds the five `.shipwright/compliance/*.md` via the existing compliance generators. The `suggest_iterate` UserPromptSubmit hook is plugin-owned (registered in `plugins/shipwright-iterate/hooks/hooks.json`); no project-level `.claude/settings.json` install is performed.

**ADR-spec-folder seeding (`trg-50efc4c8`).** `scripts/lib/adr_seeding.py::_seed_adr_spec_folder`, called from `write_agent_docs`, seeds `.shipwright/planning/adr/` — the canonical folder `/shipwright-iterate` F3 and `INDEX.md` use — with only what adopt itself minted, never with harvested third-party ADRs (`prior_art_harvester.py` copies those verbatim into `decision_log.md`/`conventions.md` instead; the two never mix). `INDEX.md` is refreshed afterward via a **subprocess** call to `shared/scripts/tools/rebuild_adr_index.py` rather than an in-process import of `shared/scripts/lib/adr_index.py` — by the time this runs, `sys.modules['lib']` is already bound to adopt's own `lib` package, and `adr_index.py`'s absolute `lib.*` imports would collide against it (ADR-045); a subprocess is a fresh interpreter with no such collision.
...
- **`env` is empty for config-layer hooks, with one deliberate exception
  (R2).** Unlike plugin-bundled hooks (which Codex documents as receiving
  `PLUGIN_ROOT`/`PLUGIN_DATA` and the `CLAUDE_PLUGIN_ROOT` compatibility
  alias), a global/config-layer hook receives no injection of its own —
  confirmed empirically (captured hook payloads showed `"env": {}`). The
  sync tool therefore rewrites every `${CLAUDE_PLUGIN_ROOT}` token to the
  bundle's literal absolute path at sync time; it never relies on the
  placeholder resolving itself. **As of R2** (`codex-plugin-execution-
  reliability`, M3), `codex_hooks_launcher.py` additionally writes one
  `SHIPWRIGHT_PLUGIN_ROOT=<bundle_root>` assignment into every generated
  launcher body, immediately before the real invocation (`set
  "SHIPWRIGHT_PLUGIN_ROOT=<root>"` on Windows; three separate
  assign/`export`/`exec` lines on POSIX, value `shlex.quote()`-wrapped) — the
  one env var `resolve_plugin_root()` already checks highest-priority, so
  `is_codex_runtime()` (see "Codex Activation Protocol" below) can resolve
  correctly under a real Codex-invoked hook. No other env var is injected;
  `command.env_clear()` on Codex's side means nothing ambient leaks in
  either direction.
...
## Codex Activation Protocol (R2/M3)

R2 (`codex-plugin-execution-reliability` campaign, sub-iterate R2, M3) adds
the first hooks that actually *enforce* something through the config-layer
shim above, rather than just running unconditionally like every pre-existing
hook. Two new hooks, registered Codex-only in
`plugins/shipwright-iterate/hooks-codex/hooks.json` (see "Runtime-scoped
hook registration (R2)" above — never read by Claude Code's own loader):
`codex_activation_mint.py` (`UserPromptSubmit`) and
`codex_pretooluse_gate.py` (`PreToolUse`, catch-all matcher within that
Codex-only manifest — see the Hooks Registry rows in the
`### shipwright-iterate` table above for their per-script behavior). This
section is the contract between them.

- **Runtime discriminator — coarse pre-check, not the real gate.** Both new
  hooks no-op immediately unless `shared/scripts/lib/codex_runtime.py`'s
  `is_codex_runtime()` is true: it resolves the plugin root via
  `resolve_plugin_root()` (`SHIPWRIGHT_PLUGIN_ROOT` > `CLAUDE_PLUGIN_ROOT` >
  `PLUGIN_ROOT`, now populated for a real Codex-invoked hook by the launcher
  injection above) and verifies BOTH bundle markers
  (`BUILD_MANIFEST.json`, `.codex-plugin/plugin.json`) are actually present
  at that path — not a bare env-var-presence check, so an ordinary inherited
  `PLUGIN_ROOT` from an unrelated shell/wrapper cannot arm anything.
  **This answers "is my resolved plugin root a real Codex bundle", never "is
  this the live, armed session I care about."** Since `~/.codex/hooks.json`
  is global, every Codex session on the machine shares the same bundle root,
  so this check is permanently true for any real Codex session once the
  launcher injection is working — it was always meant as the coarse
  "are we even under Codex at all" pre-check, not a per-session gate. Do not
  read it as R2's real gate; it never was.
- **The real per-session gate is the activation record**
  (`shared/scripts/lib/codex_activation_record.py`), keyed on
  `session_id`/`turn_id`/`cwd`/`generation`/`expiry`, stored one JSON file
  per `session_id` under `.shipwright/runtime/codex-activation/` (gitignored
  by the existing `/.shipwright/*` wildcard) plus a sibling `.consumed`
  exclusive-consume marker. `mint()` is **exclusive-create only**
  (`os.O_CREAT | O_EXCL`): the session's FIRST `UserPromptSubmit` decides
  `armed: true` (the first prompt matched the envelope grammar below) or
  `armed: false` (it didn't) — every later prompt in the same session finds
  the record already exists and mint is a no-op, by construction, not by an
  extra check. `cwd` is normalized through `shared/scripts/lib/git_base
  .main_repo_root` before storage/comparison, so a worktree-path vs.
  main-repo-path spelling of the same session's `cwd` can never split state
  across two records. `consume()` is a SEPARATE exclusive-create of the
  `.consumed` marker — a one-time claim by `codex_pretooluse_gate.py`, never
  an overwrite. **Fail-open, unconditionally:** absent, expired, corrupt,
  schema-mismatched, or cwd-mismatched all resolve to `None`/"treat as
  unarmed" — never an exception, never a deny. This is R0's Contract,
  restated: the mechanism can only ever fail toward doing nothing.
- **Envelope grammar** (`shared/scripts/lib/codex_envelope_grammar.py`) —
  the closed, ASCII, single-line wire format a terminal helper
  (`shared/scripts/tools/codex_activation_helper.py`) embeds in the first
  prompt of a fresh Codex session: `[SHIPWRIGHT-CODEX-ACTIVATE-v1|skill_id=
  <id>|args_b64=<base64url-json>]`. `parse()` never raises and rejects
  ambiguous input outright (two-or-more matches in one text → `None`, same
  as no match) rather than picking the first — closed-grammar discipline
  over text a pasted log/README could coincidentally contain. `compose()`
  and `parse()` live in one shared module so the helper and the mint hook
  can never drift apart on wire format.
- **Unarmed-launch visibility (R0 named gap 2).** The session's genuinely-
  first eligible `PreToolUse` call, when no envelope was recognized in the
  first prompt (`armed: false`), returns a non-blocking
  `hookSpecificOutput.permissionDecision: "allow"` with a
  `permissionDecisionReason` explaining the session is unarmed and the
  mandatory setup step will not be enforced — the fail-open default is never
  silent. Fires exactly once per session (the same `consume()` exclusivity
  that settles the armed matrix below); every later call in an unarmed
  session allows silently. A `SessionStart` hook could not do this instead —
  mint happens on the session's first `UserPromptSubmit`, which runs after
  `SessionStart`, so at `SessionStart` time the armed/unarmed outcome isn't
  known yet.
- **`PreToolUse` allow/deny matrix.** While armed and unsettled (the
  session's own durable first-call-settled flag, same exclusive-consume
  primitive, still unclaimed): a `Bash`-class call is tokenized (`shlex`)
  and basename-matched against `setup_iterate_worktree.py` — single-segment
  match, or a sequential (`&&`/`;`-only) chain whose every prefix segment is
  a bare `cd <path>` and whose last segment matches; any `|`/`||` anywhere
  denies outright regardless of match position (closes a compound-command-
  smuggling gap a code review found during this run: a segment-level `any()`
  match previously let `rm -rf x && uv run setup_iterate_worktree.py` pass
  in full because one segment matched). Any non-`Bash`/unrecognized local
  tool class **denies by default** while armed-and-unsettled — an
  unrecognized class must reach this hook to be denied, not bypass it via
  an unmatched matcher. A matching call consumes the settle-flag and
  allows; anything else consumes and denies, with the exact runnable setup
  command in the deny reason. Once settled, every later call in the session
  allows unconditionally. Scope: the local function-tool path only —
  Codex's hosted tools bypass `PreToolUse` entirely, a Codex-side fact this
  hook cannot change. Basename-only matching (no path/identity verification
  against the real script location) is an accepted residual risk under the
  threat model below, not a hardened boundary.
- **Threat model: cooperative local-process enforcement, not an adversarial
  boundary.** A model or process with shell access in the same workspace
  could write its own `armed`/`.consumed` state directly — the guarantee
  this protocol provides is "a cooperating-but-forgetful session can't
  silently skip the worktree-setup step," not protection against a session
  actively trying to evade it (external review, openai low-7), consistent
  with how R0 already framed envelope authenticity for the mint side.
- **Scope-broadening under Codex's merged bundle — PROJECT scope, not plugin
  scope:** unrelated to this protocol, but a hook-parity audit surfaced it
  during R2. Claude Code already fires every *enabled* plugin's hooks with no
  active-plugin filter (see "Fan-out consolidation" above) — a hook
  registered in only one plugin already fires regardless of which workflow is
  nominally active, as long as that plugin is installed, which for this
  monorepo's normal all-14-plugins setup it always is. What Codex's merged
  bundle actually broadens is reach across *projects*: its build writes one
  GLOBAL `~/.codex/hooks.json` (R1b), so a hook fires for every matching call
  in every Codex session on the machine, including sessions in unrelated
  repos — a reach Claude's per-project plugin-cache install never had. Full
  classification of every hook's tier under this reality:
  `.shipwright/planning/iterate/iterate-2026-09-20-r2-m3-hook-parity-iterate-only/hook-inventory.md`.
- **Runtime-scoped hook registration (R2):** a plugin can additionally ship
  `hooks-codex/hooks.json` (identical `{"hooks": {...}}` shape to
  `hooks/hooks.json`), read only by the Codex bundle build
  (`codex_hook_inventory.py::_load_hooks`) and merged into the built
  `plugin.json`'s hook inventory — never by Claude Code's own loader, which
  only ever reads `hooks/hooks.json`. Use it for a hook that is useful only
  under the Codex runtime (e.g. `codex_pretooluse_gate.py`, which enforces
  the worktree-activation protocol above): registering it catch-all in the
  shared `hooks/hooks.json` would have cost every Claude Code session one
  process spawn per tool call for a check Claude sessions don't need. A hook
  useful under both runtimes stays in the shared `hooks/hooks.json` as
  before — the split exists to make "zero cost under the runtime that
  doesn't need this hook" actually true, not to become the default for every
  new hook.
- **Deliberately deferred to campaign sub-iterate R2b, not built here:** a
  Stop-gated finalization-enforcement mechanism (both an iterate-only
  version and an all-phase generalization) was designed and reviewed for R2,
  then cut by Architecture Review (both external reviewers `revise`,
  ...
  `codex_completion_oracle.py`'s existing detection). Named here so a future
  reader does not assume it was overlooked; the reviewed design, this run's
  payload-capture evidence, and a recommended starting point live in the
  hook-inventory audit above.
...
### spec-reviewer / code-reviewer / doubt-reviewer (Build Step 6, Iterate Step 8)

```
orchestrator spawns a review subagent with the run_id stated in plain text
  in the prompt (write-review-payload-on-stop.py can only find it in the
  subagent's own transcript, not via SHIPWRIGHT_RUN_ID — that env var does
  not reliably reach a SubagentStop hook)
review subagent (Read/Grep/Glob only — no Write tool, unlike section-writer)
  → returns findings (JSON) as its final message — this is the ONLY copy
    until the orchestrator acts on it
orchestrator (mandate, not a guarantee): writes the reply to its payload
  file and calls record_review_pass.py BEFORE any other reasoning or
  spawning the next reviewer (iterate-2026-08-09-compaction-state-audit) —
  a compaction landing between subagent-return and this write still loses
  the finding, which is exactly what the hook below exists to catch
  → SubagentStop write-review-payload-on-stop.py fires regardless = salvage
       backstop (no-op if reviews.json already has a terminal record for
       this review_type; otherwise extracts run_id + reply from the
       subagent's transcript and records it itself)
```
...
| `external_review_state.json` | /shipwright-plan Step 5b, /shipwright-iterate (medium+) — via `mark-review-state.py`, carrying per-reviewer `verdicts` + derived `contradiction`. Current writers emit `marker_schema: 4` with `glm`/`openai` (`--driver claude`) or `marker_schema: 5` with `glm`/`opus` (`--driver codex`), both derived from the same `CURRENT_REVIEWER_ROSTERS` via `schema_for_roster()`; readers also accept historical schema-3 `deepseek`/`openai` markers, historical schema-2 `gemini`/`openai` markers (and older markers without a schema) and fail closed on mixed/triple reviewer sets. The external-review CLI likewise emits `review_schema: 2`; its historical schema 1 was implicit and used `gemini`/`openai`. | /shipwright-plan Step 6 gate (`check-plan-gates.py --gate review`), `setup-planning-session.py` resume gate, compliance `W5`, evidence collector — all three via `evaluate_review_state` |
...
| decision-drops/*.json (pending only, bounded 20 most recent) | — | — | — | — | — | — | B2 | — |
...
| events.jsonl | — | — | — | — | — | — | post-Scout bounded query (`full` explicit only) | read |
| area_catalog.json | project seed | — | plan seed | build refresh | — | — | post-Scout match + refresh | — |
...
**Iterate event-context order (P1.15):** B2 loads the non-event project
artifacts, intent/classification runs, and Repo Scout establishes changed paths
and likely FRs. Only then does `event_context.py query` select the bounded
untrusted-data bundle. Missing/stale catalog falls through to Scout path
ownership and bounded recent/global events; index failure falls through to a
bounded direct-log read. None of those degradations silently expands to full or
silently produces no readable history. Deterministic consumers in compliance,
verifiers, dashboards and finalization continue to read `events.jsonl`
directly. `full` mode is an explicit operator fallback and is counted.

**Codextender driver-selection env read.** Distinct from the artifact loads
above: at each external-review `--driver claude|codex` decision point in
plan/build/iterate (see Config File Data Flow's `CODEXTENDER_ACTIVE` row),
the executing agent checks that one environment variable in addition to
resolving which harness is driving the session. It is read at the point of
each `external_review.py` invocation, not cached at phase startup like the
artifacts in the matrix above.
...
|----------|-----------|-----------|
| `CLAUDE.md` | project | — |
| `AGENTS.md` | project, adopt | — (adopt re-runs: preserve/append, same policy as `CLAUDE.md`). Codex CLI reads this as its own native convention, not a `CLAUDE.md`-absent fallback. Single-source: both producers render the exact same shared body as `CLAUDE.md` (`host_name="Codex"` swap for the one host-specific sentence) and append `shared/templates/codex-agents-md-appendix.md` verbatim — the only Codex-specific content, so there is one file to keep in sync, not two generators. `scripts/lib/agents_md_renderer.py` (adopt); `project-scaffolding.md` step 2 (project, agent-instruction-driven). iterate-2026-09-23-m5-agents-md-generation-drift. |
| `CONTEXT.md` (target project root, domain glossary) | `/shipwright-project` interview (`shared/scripts/tools/write_context_term.py`, one `Language` entry per sharpened term, `file_lock` + `durable_atomic_write`) | the same producer on every re-sharpen (idempotent upsert; `Relationships` / `Flagged ambiguities` preserved but never written — no producer for either yet, hand-edit only, never interleaved with a running interview's writes). Format SSoT: `shared/context-format.md`. **Read via `shared/scripts/tools/context_md_format.py`'s `read_terms()`** — the sanctioned way to read sharpened terms back (never import the write-side internals or re-derive the parser); matching is exact-case, exact-prose, no folding. **Read by** `shared/scripts/tools/verify_grill_trace_completeness.py` (P4.2's grill-trace completeness gate, see the `grill-traces/<requirement_key>.json` row below) — the consumer this API was built ahead of. |
| `{planning_dir}/grill-traces/<requirement_key>.json` (P4.2, `shared/grill-trace-format.md`) | `/shipwright-project` interview (`shared/scripts/tools/write_grill_trace.py`, one file per elicited requirement, written at confirm-before-acting — `requirement-elicitation.md` §9 — via `--payload-file`, `file_lock` + `durable_atomic_write`, same shell-injection discipline as `write_context_term.py`) | the same producer on every re-sharpen of that requirement (idempotent upsert, keyed by `requirement_key`; never touches another requirement's file). Read by `shared/scripts/tools/verify_grill_trace_completeness.py` — the **completeness gate**, registered as `check_grill_trace_completeness` in `shared/scripts/tools/verifiers/project_checks.py::run_project_checks()` (the SAME code-level dispatcher C1-C5 use) so `plugins/shipwright-run/scripts/lib/phase_validators.py::_validate_project` re-runs it on every `update-step --step project` call and genuinely BLOCKS phase completion on the four closed-vocabulary STOP conditions the design names — not merely the Step 8 prose (`plugins/shipwright-project/skills/project/references/step-8-completion.md` item 7) instructing the agent to run the CLI and decide for itself; that prose now documents the code-enforced gate rather than being its only enforcement (blank dimension, greenfield `assumed`, undefined term, outcome without a fit criterion) plus four separately-named structural/consistency guards (`grill_trace_coverage` — an interview ran but zero traces exist; `fr_trace_coverage`, `shared/scripts/tools/grill_trace_fr_coverage.py` — a live FR row with no matching trace, catching a partially-recorded interview; `glossary_source_available` and `glossary_delta_declared`, `shared/scripts/tools/grill_trace_glossary.py`). The undefined-term check reads `shared/glossary.md` (line-anchored bold-bullet parse) union `CONTEXT.md` via `shared/scripts/tools/context_md_format.py`'s `read_terms()` — the same sanctioned reader `CONTEXT.md`'s own row above names as the intended future consumer. Only `/shipwright-project`'s surface is wired as of P4.2; `adopt`/`iterate` are reserved surfaces in the schema, deliberately not wired here (`trg-1aa5a8ab` owns the onboarding-side trigger). Never judges prose quality — structural completeness only (`shared/grill-trace-format.md` §3). |
...
| `.shipwright/agent_docs/decision-drops/INDEX.md` | `shared/scripts/lib/decision_drops_index.py` | Local-only convenience index of pending (not yet folded) decision-drops, refreshed by `write_decision_drop.py` (a new drop was added) and `aggregate_decisions.py` (drops were folded and deleted). **Deliberately gitignored, unlike the ADR/decision-log indexes and unlike the directory it lists (tracked, see the row above)** — this render alone stays local-only, so it carries no `CHURN_ALLOWLIST` entry (git can never see a conflict on an ignored path) and has no CI drift guard against a committed copy. Each worktree now has its own decision-drops directory (no shared main-repo resolution to contend on); the file lock guards a refresh racing this run's own concurrent write within the SAME checkout, not cross-worktree contention. Refresh by hand with `uv run {shared_root}/scripts/tools/rebuild_decision_drops_index.py --project-root .`. |
| project-declared published package manifests (e.g. `bootstrapper/package.json`) | `/shipwright-changelog` Step 5.4 (`sync_release_manifests.py`, only for paths declared in `shipwright_changelog_config.json`) | Verified against the COMMITTED blob (`--verify-commit`, Step 6, chained before `git tag`) — never the worktree, so an omitted pathspec still fails the release rather than tagging over a stale manifest. Full contract: `plugins/shipwright-changelog/skills/changelog/references/manifest-sync.md`. No manifests declared = no writes, no reads beyond the config file itself. |
| `.shipwright/runtime/manifest_sync_result.json` (gitignored, disposable) | `/shipwright-changelog` Step 5.4 | Step 6's `--verify-commit --result-file` call — the frozen manifest list Step 5.4 actually wrote/staged, read instead of re-deriving scope from the (by-then mutable) config file. |
| `.shipwright/runtime/release_notes_v{version}.md` (gitignored, disposable) | `/shipwright-changelog` Step 7 (`publish_release_notes.py`, after the tag is pushed) — extracts the just-tagged `CHANGELOG.md` section from the tagged git blob (`extract_changelog_section.py`), condenses it via a single tool-less LLM completion (`condense_release_notes.py`, no function/tool definitions passed, so injected CHANGELOG content can influence only the returned text, never take an action), then runs the result through the deterministic `validate_release_notes.py` gate (fixed heading vocabulary, size cap, no raw HTML/bare URLs/off-repo links) before writing the sanitized body here | `create_github_release.py` (`gh release create --verify-tag --notes-file`) publishes a GitHub Release from this file's exact bytes. Best-effort, never blocking: every stage reports a distinct status (`ok`/`exists`/`skipped: <reason>`/`failed: <reason>`) into the Step 7 summary line, and a failure at any stage never fails the changelog phase. Forward-only (pre-existing tags are not backfilled). Rationale: `plugins/shipwright-changelog/skills/changelog/references/release-workflow.md`. |
| `architecture.md` | project | write_decision_log.py (architecture impact) |
| `.shipwright/agent_docs/area_catalog.json` | shared `area_catalog.py` producer, triggered by project (greenfield) or adopt (brownfield) | the same producer, triggered by plan (planned paths), build (realised paths), and iterate (unmapped provisional paths). One schema/writer; `.shipwright/adopt/derived-catalogue.json` remains a separate requirements-honesty artifact. |
| `build_dashboard.md` | update_build_dashboard.py | build, test, changelog, deploy, iterate, **Stop hook** (all plugins) |
| `session_handoff.md` | generate_handoff_on_stop.py | all plugins (Stop hook), **finalize_iterate.py** (iterate). Section renderers live in `shared/scripts/lib/`: `handoff_iterate.render_iterate_progress` (in-flight ITERATE state — reads `.shipwright/planning/iterate/{run_id}/reviews.json` via `lib.review_record_core`, iterate-2026-08-09-compaction-state-audit, and reports `not started`/`not due yet`/`interrupted`/`complete`/`unreadable`; this is a secondary, best-effort convenience — B1's own resume check reads `reviews.json` directly, not via this file) and `handoff_pipeline.render_pipeline_phases` (**`## Pipeline Phases`** — FR-01.01: which phases are finished, which one was interrupted, which failed, and the loop's dispatch pointer). The finished tally denominates against `run_config.pipeline` (falling back to the task count, and taking whichever is larger so splits are not undercounted) — NOT against `len(phase_tasks)`, which is materialised one task at a time and made a run one phase into seven read as "1 of 2". The pointer is only called *dispatched* when the attempt counter is >= 1 or the pointed task is `in_progress`; `advance_pointer` parks the pointer on an undispatched successor with attempt 0 after every completed phase, so pointer-set alone is the normal between-phases state and is rendered as **Next up**. The pipeline block is rendered from state the run already holds — `run_config.phase_tasks[]` (authoritative, mutated only via `phase_task_lifecycle`) plus `.shipwright/run_loop_state.json` — and is **absent entirely** for any config without `phase_tasks[]`, so legacy / standalone / adopted handoffs are unchanged. It sits ABOVE the event-derived `## Recovery` tally (which counts distinct `phase_completed` events) and the legacy checkpoint block, because it is the authoritative view of the two. `shared/` must not import from a plugin, so the loop-state path literal is duplicated; `integration-tests/test_handoff_reads_real_loop_state.py` reads the owner's constant in a subprocess and fails if the two drift. |
...
| `.shipwright/runtime/events-context-index.json` | `shared/scripts/tools/event_context.py` | the same tool; disposable, gitignored, fully rebuildable from the raw event log + area catalog + git history (`shared/scripts/lib/commit_trailers.py` backfills `commit`/`changed_files` from each commit's `Run-ID:` trailer when the event itself declares neither — see `## Architecture Review` on `iterate-2026-08-07-events-context-backfill-keys`). Cache validity checks the event-log fingerprint AND a re-resolved git base-ref sha against `coverage.commit_map.resolved_sha` — a new commit landing with no event appended (F5b writes `work_completed` before F6 commits) invalidates the cache on its own. `coverage.fields.<key>` splits its `unavailable` population into `not_applicable` (the entry has no run_id/commit linkage at all — an observation-type event, e.g. `grade_snapshot`, structurally cannot carry the key) and `missing` (the entry IS a linked change record but the key is still empty — actionable). `supersedes_event_id` is deliberately excluded from that split and stays `not_applicable`-only: not superseding a prior event is the normal state for nearly every entry, so there is no data-derivable "should have had this" signal for it (iterate-2026-08-08-coverage-envelope-split). `coverage.missing_work_completed` lists the (capped, `truncated`-flagged) `work_completed` event ids with at least one genuinely-missing field, for direct follow-up. Never an audit source. |
| `.shipwright/compliance/context-cost/events-context-{metrics.jsonl,report.md}` | `event_context.py query` (temporary P1.15 observation) | query replaces the current run's one metrics line and regenerates the latest/rolling operator report. Neither artifact is loaded into agent context; removal waits for a separate operator-decided cleanup after 5–10 normal compact iterates. |
...
read for F5). Since iterate-2026-08-08-track-decision-drops tracked the
directory, each worktree has its own decision-drops (no more shared-disk
cross-contamination between sibling worktrees), but a campaign's sub-iterates
share ONE worktree and branch-hop inside it, so a prior sub-iterate's
uncommitted drop can still be sitting on disk when a later branch is checked
out — scoping stops that leftover drop from false-flagging drift on a branch
whose own target-doc entry never named it. Fail-open when no event log
exists, so a clean CI checkout (drops dir absent anyway) keeps whole-set
behavior (iterate-2026-06-12-arch-drift-test-scope).
...
| **xdist** | **Per-unit OPT-IN** via `suite.xdist` in `shipwright_test_config.json`. A global `-n auto` is FORBIDDEN. **`shipwright-compliance` must stay off the allowlist** — it is not xdist-safe (shared-state races in `test_test_evidence.py`). **`integration-tests` is also deliberately serial inside its outer unit**: cache-fanout tests already spawn up to twelve subprocesses, and nested xdist under outer fleet load reproduced 17 cache-ready failures although the root passed alone with xdist. |
...
| **Faults and process trees** | An infra fault is retried ONCE with the **identical command shape** (xdist still on). A deterministic fault (rc 5 nothing-collected, usage error, unprovisionable xdist) reproduces and still FAILS — nothing is laundered. A transient one recovers. **The retry must never strip xdist.** A hang is capped by `suite.timeout_seconds` (default 1800) and becomes a fault. A dedicated reader continuously drains combined bytes into a fixed 64-KiB tail and publishes only that tail to the owned attempt file, bounding pipe pressure, memory, and disk; JUnit existence, not output, classifies whether pytest ran. On Windows a handshake-blocked launcher enters a kill-on-close Job Object before it may spawn uv; POSIX uses a process group. Timeout, cancellation, and normal completion terminate/reap remaining descendants before owned temp state and leases unwind. |
| **Failure evidence** | Every failed attempt atomically publishes a byte-capped, UTF-8-replacement-decoded, credential-redacted JSON record labelled `untrusted` under the canonical main repository's gitignored `.shipwright/runs/<safe-run-id>/f0-diagnostics/`. Linked worktrees resolve that stable root before writing, so F11 worktree cleanup cannot orphan the path. A race card carries only that repository-relative `evidencePath`, never the output itself in tracked triage. Evidence-publication failure cannot hide an already-red verdict and stops an otherwise-recovered run. |
| **Host resources** (iterate-2026-08-03-f0-host-resource-lease; P1.16 fleet calibration) | Outer pool + inner xdist workers share ONE per-run budget. A repository-keyed weighted OS-lock lease caps the sum granted across sibling worktrees at the hardware budget (`cpu_count - 2`); compatible F0s overlap, exhausted capacity waits strict FIFO with owner/run-id heartbeats, and process death releases capacity without stale-file deletion. Oversized `suite.max_workers` is capped. This repository requests **11** against measured capacity **22**, selected from candidates 8/11/22 by four-run fleet makespan and queue-time replay, so two siblings can run concurrently without oversubscription. An exclusive sibling-worktree uv lease wraps only xdist provisioning + warm-up and is released before the CPU lease, so lock order is non-nested. The existing `.coverage.f0.lock` remains per-worktree and still rejects a second F0 in the same checkout. |
...
existing drift) cannot diverge. Decision-drops are tracked (since
iterate-2026-08-08-track-decision-drops) but per-checkout — F5 `skip`s
whenever the calling checkout has nothing pending (a fresh clone, or a
post-release dev tree with the backlog already folded) — the F11 gate is the
authoritative prevention layer regardless. Replaced the dead, mtime-only
