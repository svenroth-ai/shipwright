...
**Option C -- Skip external review, rely on the internal reviewer:** Leave keys unset. `/shipwright-plan` and medium+ `/shipwright-iterate` will prompt you at Step 5. For `/shipwright-plan`, an internal Opus reviewer (`opus-plan-reviewer`) already ran before the prompt and carries the gate if you skip; the mandatory self-review ("2x denken") pass only runs as a last resort if that internal reviewer itself could not run. For `/shipwright-iterate`, skipping falls back directly to the mandatory self-review ("2x denken") pass. Either way, the opt-out decision is logged in the decision log and `external_review_state.json`, so audits still see it. Silent skipping is no longer possible.
...
| `/shipwright-iterate` | `"description"` | `--type feature\|change\|bug`, `--complexity trivial\|small\|medium\|large`, `--review`, `--pause`, `--campaign <slug>`, `--sub-iterate-id <id>`, `--autonomous`, `--review-model opus\|sonnet\|haiku\|inherit`, `--finalization-model opus\|sonnet\|haiku\|inherit` | Complexity-adaptive SDLC for ongoing changes. Auto-detects intent and complexity, scales phases from quick fix to structured mini-pipeline with planning, review, and testing. Campaign mode (`--campaign`) groups related sub-iterates; `--autonomous` runs them **interleaved-serial** -- each sub-iterate is its own PR, merged after CI-green before the next builds from a fresh `origin/main` (`branch_strategy: serial`, the default), so shared-file edits compose with no end-stage merge drain. `--campaign <slug> --sub-iterate-id <id>` on a single hand-run sub-iterate stamps `campaign`/`sub_iterate_id` into its `work_completed` event (same self-identification the autonomous runner records). `--review-model`/`--finalization-model` override the Claude model tier the review cascade / `sub-iterate-runner` spawn on, for this run only — unset (or an explicit `shipwright_model_config.json` at the project root) defaults to `inherit` (today's behavior: follows the session's own model). |
...
| `/shipwright-plan` | `@spec.md` | -- | Create implementation plan for one split. Researches stack, interviews for clarification, generates section files. Always reviews the plan with an internal Opus reviewer first; optionally sends it to external LLMs (DeepSeek + OpenAI) for review on top of that. |
