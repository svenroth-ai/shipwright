...
- **Operate from CLI** (first-class, parallel to the WebUI Triage tab):
  ...
  - `uv run shared/scripts/tools/triage_cli.py amend trg-XXXXXXXX [--title T] [--detail D] [--severity S] [--kind K]`: correct a card's title/detail/severity/kind IN PLACE — the id, status, and audit history are untouched; at least one field is required. Use this instead of dismiss-and-refile when a card's *content* was wrong, not its disposition.
