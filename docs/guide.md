...
0. **Fresh verification (F0)** -- run the full canonical unit/type/integration suite; sibling worktrees share the host CPU and uv warm-up leases, and the runner emits progress heartbeats during long units. Diagnostic file shards are allowed only after an interrupted run and never count as F0 completion; the full canonical runner must still exit zero. Any non-zero exit means STOP, so do not enter F0.5.
