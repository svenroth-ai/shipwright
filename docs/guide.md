...
13. **Watch it to delivery** -- arming auto-merge is not the same as having delivered. A Required Check can fail *after* the arm, leaving the PR sitting un-merged and red while the run walks away: shoot-and-forget. So finalization's last step watches the PR (`shared/scripts/tools/watch_pr_delivery.py`) until it is genuinely **merged**, a Required Check fails (STOP -- diagnose, fix, re-push, re-watch), or it is closed. While the PR is still open it reports **why it is not merging** -- conversations still unresolved, required checks that never reported at all, and the code host's own verdict that the merge is blocked -- rather than only how long it has waited; anything it could not check is reported as unchecked, never counted as clear. Exit codes: `0` merged, `2` a Required Check failed, `3` closed, `4` still pending when the watch timed out. Only `0` means delivered.
