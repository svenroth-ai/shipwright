...
- **Two-stage review pattern:** a `spec-reviewer` spec-compliance HARD-GATE runs first, then the `code-reviewer` quality stage (`plugins/shipwright-build/agents/spec-reviewer.md`; build Step 6 / `references/code-review.md`). An optional third `doubt-reviewer` (fresh-context, disprove-biased, addyosmani/agent-skills OS3) runs after for non-trivial diffs. When the driving harness cannot spawn an independent Agent-tool subagent for any of these (Codex CLI itself driving, or Claude Code redirected to a non-Anthropic backend), the cascade runs through `review_via_codex.py` instead — see `shared/prompts/codex_review_dispatch.md`.
...
- **`/shipwright-iterate`** finalization pushes its branch and opens a PR per change, and gets that PR merged -- by asking GitHub to when it can, or by merging it itself when GitHub cannot (see §2.10).
...
With branch protection in place, `/shipwright-iterate` PRs can merge themselves the moment every Required Check is green: no bot to run the merge, just GitHub-native auto-merge. (The run still watches the PR afterwards to confirm it *actually* merged -- see step 13 of [Chapter 8's Finalization](#8-ongoing-development-with-shipwright-iterate). Arming is not delivering.) `/shipwright-adopt` writes an **`AUTOMERGE_SETUP.md`** into your repo listing the exact Required-Check names *your* workflows produce; the steps below are the general flow.

> **You do not have to set this up to get your changes merged.** GitHub can only be *asked* to merge a PR automatically when the branch it merges into is protected -- and a private repo on the free plan cannot have branch protection at all. Where GitHub refuses, Shipwright waits until every check that exists has passed and then merges the PR itself, so a change still ends up in your main branch without you doing anything. It always tells you which of the two happened, and how many checks the code host actually ran -- if that number is zero, the tests that ran on your own machine are the only evidence, and the report says so rather than implying GitHub confirmed anything. Prefer nothing ever merged on your behalf? Set `SHIPWRIGHT_ITERATE_SELF_MERGE=0` and Shipwright will only ever ask GitHub; a change it cannot get merged is then reported as not delivered straight away, for you to merge by hand. Setting the protection up anyway is still worth it: it is what makes the checks *required* rather than merely present.
...
4. **Artifact generation.** Writes `CLAUDE.md`, `.shipwright/agent_docs/{architecture,conventions,decision_log,build_dashboard}.md`, `.shipwright/planning/01-adopted/spec.md`, the six `shipwright_*_config.json` files, `shipwright_events.jsonl`, and `e2e/flows/adopted-baseline.spec.ts` (regression guard from the crawl). It also merges a managed `.gitattributes` fragment: `merge=union` keeps concurrent append-log updates (`shipwright_events.jsonl`, `.shipwright/triage.jsonl`), while `-text` preserves immutable per-run test-result bytes during staging (an existing `.gitattributes` is preserved; only missing rules are appended).
5. **Traceability baseline.** Scaffolds the `@FR` test-tag convention into `.claude/rules/`, runs the shared backfill engine to tag the repo's existing tests, and takes a repo-wide inventory of every pre-existing skipped/focused/quarantined test. Orphan candidates (a test tagged with a requirement that no longer exists), untagged/unmapped tests, and standing skips are filed as triage items so they land in the Inbox rather than being lost — an unmapped test is flagged as a review candidate, never accused of being stale. Requirements whose layer coverage the scan cannot infer are asked about interactively (or resolved from a predeclared answer set in an unattended run, which never stalls). A repo with no tests still onboards cleanly.
5b. **An honest baseline — what was derived, and what was inherited.** The requirements catalogue Adopt writes was read out of your code; **nobody has confirmed it**. So the generated `spec.md` opens its requirements section by saying exactly that, with the count, and the same facts land machine-readably in `.shipwright/adopt/derived-catalogue.json` — because traceability, coverage and drift all measure against that catalogue, and a catalogue nobody has confirmed must not look like one somebody did. The adoption commit and the handoff banner report the number too, and Adopt files a follow-up in your Inbox asking you to work through the requirements **with a person** (following the shared questioning method in `shared/requirement-elicitation.md`): reading the code is a start, and it is not enough on its own. Alongside it, `shipwright_known_failures.json` records what your codebase *arrived* with — tests that were already failing, requirements with no test, and tests that are switched off — as **inherited**, so they are never counted as your project's own failures. Those gaps become tracked follow-ups instead of blocking a run. A baseline test run that was never made is recorded as *not measured*, never as "zero failures".
6. **Compliance seeding.** Generates `.shipwright/compliance/{sbom,change-history,traceability-matrix,test-evidence,dashboard}.md` plus the requirement→test manifest (`test-traceability.json`, layer-aware, populated from the baseline tags) via the existing compliance infrastructure.
7. **Workflow scaffolding + automerge guide (CI + Security + CodeQL + Claude-Review).** Lands dormant GitHub Actions workflows in `.github/workflows/`: profile-specific `ci.yml` (with the cross-platform OS matrix `ubuntu-latest` + `windows-latest` as the default), `security.yml` (Semgrep + Trivy + Gitleaks scanner chain), `codeql.yml` (GitHub-native SAST; the `language:` matrix is rendered per detected stack profile, and `continue-on-error` on the analyze step keeps the `Analyze (<language>)` check green on a private repo without GitHub Advanced Security), and `claude-review.yml` (independent Claude Code review on PRs). All are byte-equal idempotent: pre-existing workflow files are preserved bit-for-bit. CI + Security + CodeQL ship dormant (`workflow_dispatch:` only); Claude-Review fires on `pull_request` by design. Adopt also writes a repo-root **`AUTOMERGE_SETUP.md`**, a profile-aware branch-protection / auto-merge guide whose Required-Check job names are *derived by parsing the actually-deployed workflows* (matrix-expanded; conditional `if:`-gated deploy jobs are flagged, not listed as requireable), so a brownfield repo can be wired for GitHub auto-merge without trial-and-error. (Anti-ratchet `bloat-check.yml` is a deferred follow-up.) See [docs/security-ci-setup.md](security-ci-setup.md) for the free Path-A scanner chain vs. CodeQL/GHAS Path-B trade-off.
8. **Layer 3, external LLM review.** Runs `llm_review.py` over the generated artifacts to flag hallucinations or contradictions (skipped gracefully if no API key is set).
9. **Validation + commit.** Runs `validate_adoption.py`, then a single Conventional Commit `chore(shipwright): adopt repository into Shipwright SDLC`.
...
- **Confirm the requirements with a person.** Adopt leaves this in your Inbox as `adopt-derived-catalogue-confirmation`, and it is the highest-value thing you can do next. Every requirement in the catalogue was derived by reading code: plausible, and unconfirmed. Until someone who knows the product works through them, every traceability and coverage number describes the catalogue rather than the product. Shipwright's own repository is the cautionary tale: its catalogue came from onboarding, and a whole campaign later existed to repair it.
...
**Purpose.** Creates a detailed, section-based implementation plan from a single spec file. The plan follows a TDD approach (tests defined before code) and is **always independently reviewed first by an internal reviewer** (`opus-plan-reviewer`; model resolved from `plan_review` in `shipwright_model_config.json` — inherit unless a project configures it), then **by external LLMs by default on top of that** (ZDR-routed GLM 5.3 + GPT-5.6 Terra). If no API key is set in `.env.local`, the skill **asks you interactively** whether to add one or skip and rely on the internal review, which already carries the gate; a mandatory self-review ("2x denken") pass remains only as a last-resort fallback for the rare case the internal reviewer itself could not run. It no longer silently skips review.
...
**What it needs.** A `spec.md` file generated by `/shipwright-project`. Recommended: `OPENROUTER_API_KEY` in `.env.local` at the repo root (one key covers both review models). A direct `OPENAI_API_KEY` can run the GPT arm only; GLM always requires the approved OpenRouter ZDR route. If no key is set, the skill will prompt you at Step 5.

**What it produces**

- `plan.md` -- the implementation plan with a `SECTION_MANIFEST` block that lists all sections in build order. A section that needs what another produces names it (`03-api: 01-auth, 02-database`), which is what makes the order checkable rather than merely intended
- `sections/01-name.md`, `sections/02-name.md`, etc. -- self-contained section files, each describing one unit of work for `/shipwright-build`. Each names the requirements it serves on a `Requirements:` line, says what it is for, lists at least two implementation steps, and states how it will be tested
...
- First has an internal reviewer (`opus-plan-reviewer`) independently check the plan against the spec -- every finding is fixed, disclosed as a known limitation, or declined with a reason, and a declined or disclosed high-severity finding is put to you before the plan moves on. Then sends the plan to GLM and OpenAI in parallel for external review by default, presents findings, and lets you accept or reject each. If no API key is available, the skill stops and asks whether to add one or rely on the internal review that already ran; the mandatory self-review ("2x denken") pass is a last-resort fallback only for the rare case the internal reviewer itself could not run. The outcome is written to `external_review_state.json` so the compliance dashboard can show it
- **If the two reviewers disagree about the plan as a whole** -- one endorsing it while the other says the approach is wrong -- that is recorded as its own outcome and put to you to decide, and splitting the plan into sections refuses to start until your decision is on record. The same applies when a reviewer's conclusion could not be read, or when only one of the two answered: neither is agreement. Two reviewers exist so a disagreement gets noticed, which a single count of findings cannot show
...
- Validates, as a command rather than a checklist (`check-plan-gates.py`), that every functional requirement is covered by at least one section, that every section traces back to a requirement it serves (so a plan cannot quietly add work nobody asked for), that each section states purpose/steps/tests, and that no prerequisite is numbered after the section that needs it. The same four gates run again when the phase is marked complete
...
5. Runs a smoke test against the deployed URL. It keeps asking until the app answers or until the deadline runs out, and **the deadline belongs to the target** -- it comes from that target's deploy profile (`poll_interval_seconds` / `max_wait_seconds`; 5s and 60s for Jelastic). An application that needs fifteen seconds to start is not a failed release.
...
**What a rollback promises.** Going back to a named version actually sends that version to the target -- it does not simply redeploy whatever is currently at the top of your branch -- and it reports success only for a version the request really carried. Where the target can be asked which version it is now running, the answer is passed on: confirmed, or *unconfirmed* in those words, never a confident claim the target never made.

Before restoring old code, it checks whether your **stored data has already moved on**. If database migrations exist that the older version does not know about, the rollback stops and tells you which ones, along with what that target's profile says about data (for Jelastic: down-migrations). Restoring code into a database it was never written for is a worse outcome than a paused rollback, so continuing is a deliberate choice you make, not a default.

And if the rollback itself fails after it has begun changing the target, it says so in plain words -- that neither the new nor the old version is confirmed running, what it last tried, what that returned, and what it had already changed -- then stops rather than continuing unattended. This is deliberately distinct from a rollback that was refused before the target was touched, which changes nothing and says that too. The worst case is not that a rollback fails; it is that nobody notices.
...
- **D:** Implementation evidence. Every FR has at least one `work_completed` event (D5 enforces FR-or-`spec_impact=none` linkage), every built section has `test_count > 0`. **D-orphan** flags a `@FR` test tag that names a removed/unresolvable FR (MEDIUM hygiene once confirmed orphaned; a merely possible orphan or an invalid tag is LOW; a `## FR-Fold-Map` entry can rescue it — see Section 4.6). **D-layer** flags an FR still missing an executed-passing test in a layer its spec requires.
...
- **I:** Requirement hygiene vs `shared/fr-authoring.md`. Flags FR names and descriptions carrying implementation detail (file paths, ADR numbers, HTTP verbs, code symbols), rows that only describe a change to another FR (fold candidates), duplicate/reused FR IDs, criteria that exist but are not in the prescribed `(E) Given ... when ... then ...` shape (**I7**, §3b), a `/shipwright-adopt` `TBD` acceptance-criteria placeholder open 90+ days (**I8**, read from git history, no stamped state), and a requirement whose recorded changes never co-occur with a decision-drop / ADR run_id (**I9**, "Rewritability" — REQ3.04c M7; a proxy, not a text link, since neither the FR catalog nor the decision-drop schema carries a direct field for it — see `shared/scripts/lib/rewritability_links.py`). I1-I3, I6-I9 are **advisory** — reported with counts but never changing the audit verdict, so a spec with legacy prose can clean up gradually without reddening CI. I4 (duplicate FR ID) and I5 (malformed `Basis`) fail for real. **A row a running iterate itself adds or edits is held to I1/I2/I7 for real**, though: the non-dodgeable F11 gate `check_fr_hygiene_on_touched_rows` recomputes which FR rows a run's diff touched and blocks finalization on a hit — narrower than making the advisory checks blocking everywhere, and it does not reach untouched legacy rows (`docs/hooks-and-pipeline.md`).

> **Detective-only tagging.** Every Group F4–F7, Group H and Group I finding is tagged `SOURCE_DETECTIVE_ONLY` so the audit report distinguishes them from preventive re-runs of F1–F3 / C / B3 / B6. An on-demand run (`/shipwright-compliance`, `run_audit.py`) reports and writes its own local audit record only; backlog convergence into `.shipwright/triage.jsonl` (source `compliance`, auto-dismiss when drift resolves) happens solely under delivered-merge or verified-release authority — see "Compliance backlog lifecycle authority" in `docs/hooks-and-pipeline.md`.
...
- `shipwright_compliance_config.json` → `last_audit`: the audit's own run record (`ran_at`, `verdict`, `scope`, `checks`). Unlike the two reports it is **tracked**, which is what lets every compliance document say when the cross-check last happened — see *Freshness disclosure* below. A `--only` run records its group letters as the scope, so a partial audit can never be read as a full cross-check. Recording is best-effort: it never changes the audit's exit code, and a config it cannot parse is reported and left untouched rather than overwritten.

**Freshness disclosure — "when was this last checked?"** The detective audit is deliberately **on demand**: it has no schedule, no CI trigger and no hook, because a check nobody asked for produces warnings nobody wants. The cost of that choice is that in a project where nobody thinks of it for months, divergence accumulates while the evidence documents look unchanged and trustworthy the whole time. So the documents disclose the date instead of hiding it. Every evidence document carries the answer as the third line of its provenance header — beside *when* it was generated and *which state* it describes (card `trg-4d5b6a56`), the `Consistency-audit:` line says whether anything ever checked that the state named there matches reality: `Consistency-audit: last run 2026-07-15 (12 days earlier) — PASS`, or simply `Consistency-audit: never run`. The dashboard's **Consistency Audit** section then states it in full with the check counts. Ages are measured against the document's own event-pinned generation reference, never the wall clock, so regenerating a document twice yields byte-identical output. A partial (`--only`) run never displaces the last whole-project check — the documents lead with "last full run …" and report the partial one alongside it — and a record that is present but damaged reads as *unknown*, never as "never run", because that would assert something the project cannot know. After every audit `/shipwright-compliance` regenerates all five markdown documents (Step 2b) so the answer it just recorded is visible immediately. The point is not to nag: it turns *possibly-never-checked* into *visibly-not-checked-since*, so a reader can weigh the evidence for themselves.

**What the auto-background generator produces:**
- `.shipwright/compliance/dashboard.md` -- Start-here overview. It **opens with a Control Verdict + Control Grade (A--F) block**: a plain-language verdict plus a deterministic, importance-weighted control-posture grade computed by `lib/control_grade.py`. The scoring follows **OpenSSF Scorecard's** published methodology -- a weighted average over the *measurable* dimensions, where a not-applicable dimension is **excluded from the denominator** (Scorecard's `-1` "inconclusive" convention) rather than scored 0, and an all-N/A repo renders **"Not Gradeable"** instead of an unearned `F`. Each of the seven dimensions names -- in its **Anchor** column -- the recognized **open** standard it follows (no commercial-vendor tool is cited; the plain-language map of *what each dimension checks* and *what each standard is* lives in the table below), and a `Verified from:` line stamps the event log + commit range so the grade is reproducible (the same scorer later powers the public repo-grader). **The headline can never outrun the weakest control:** the verdict is held below "A — full control" whenever a load-bearing pillar is *expected but unmeasured* (e.g. the CI security gate isn't running) or outright broken — so a green headline can never mask a control that has gone dark, while the per-dimension table keeps the full detail. **Workload composition is deliberately grade-neutral:** the *mix* of feature vs. maintenance/hardening work does not affect the grade — a repo doing correct, honest maintenance (few new FR tags) is fully in control. "Control" is *is every requirement's state known and current, and is every change honestly attributed*, not *what fraction of recent work happened to be features*; that traceability control is carried by requirement coverage and change reconciliation (both composition-independent) and enforced structurally by the fail-closed FR-gate at write time, never by the recent FR-tagging *rate*. The **change-reconciliation** dimension is measurable from the per-FR impact map (`fr_impact`, documented under the traceability matrix below): it scores the share of behavior-affected requirements that have been re-verified since they were last touched, and is age-neutral -- an old but re-verified change is fully reconciled, while a behavior change left unverified is the gap it surfaces. The **security** dimension is lit from the **CI gate**, not a local re-scan: a producer (`refresh_ci_security.py`) pulls the latest `security.yml` run's `findings.json` and writes a tracked, **public-safe** summary (`.shipwright/compliance/ci-security.json` -- scan date, findings-by-severity, critical-gate pass/fail, and the accepted-via-`.trivyignore` register with expiry; counts + verdict only, no finding detail), rendered as a **CI Security** section below the grade block. Open high/critical from that summary feed the dimension; when no trustworthy CI summary has been ingested it stays **n/a** rather than reading the stale, FP-laden local `securityreports/` as a false CRITICAL (this is also what surfaces security on repos -- e.g. the WebUI -- that keep no local scan reports). The block renders for both adopted and greenfield projects. Then quality indicators, project velocity, and links. Two readability fixes ride along: the **"All unit tests passing" headline reads the latest *full* suite** (not the last event -- often a doc commit -- so a real suite total like `3473/3473` is shown, never a misleading `0/0`), and a **Consistency Audit summary is inlined** rather than linking the gitignored `audit-report.md` (which 404s on public GitHub) — rendered from the tracked `last_audit` record, so it reads identically on a developer's machine, in CI and on the public repo, and names when the cross-check last ran or states plainly that it never has. Mode-aware: adopted projects (`run_config.adoption` set) render `Pipeline phases completed: n/a (adopted)` instead of a fake `1/7 WARN` and hide the structurally-N/A `Work events (build)` + `All sections reviewed` rows. Every WARN row carries a 4th `Why warn?` column with a one-line diagnostic pointer (`"X failing — see test-evidence.md"`, `"see sbom.md"`, ...). A `Triage open` indicator surfaces the count of open items in `.shipwright/triage.jsonl` (signal severities prominent, info-severity shown in parens, `"3 open (1 info)"`) so the inbox status lives on the compliance home page. A **`Recent changes traced to an FR`** indicator shows the share of the most recent changes that carry an FR tag -- **informational only**: it makes the feature-vs-maintenance mix visible but never WARNs and never affects the grade (composition is work type, not control). (This measures genuine FR-linkage; the requirement-traceability grade dimension credits both FR-linked and explicitly-classified no-FR work -- see below.) **Bloat column:** the per-phase quality matrix carries a Bloat indicator sourced from `shipwright_bloat_baseline.json`: green when no entry's measured LOC exceeds its frozen `current`, WARN when any file ratcheted past its limit, with a one-click pointer to the Group H detective audit.
...
- **Producers:** `audit_phase_quality_on_stop` (Tier-1 FAILs, 24h dedup), `audit_detector.mirror_findings_to_triage` (only from an authoritative delivered-merge or verified-release lifecycle audit; the Stop hook reports branch-local compliance diagnostics but never writes the rolling backlog), `generate_security_report._emit_findings_to_triage` (security-report runs), `performance_check._emit_failures_to_triage` (perf-gate runs), `check_drift._emit_drift_to_triage` and `artifact_sync._emit_drift_to_triage` (drift hooks), and the throttled SessionStart hook `import_github_findings.py` (GitHub action-units, see § 4.11.1 below).
...
- **Operate from CLI** (first-class, parallel to the WebUI Triage tab):
  - `uv run shared/scripts/tools/triage_cli.py list`: list open items + their `launchPayload`, followed by any **deferred** items in their own section (id, severity, revisit date, reason, title — no payload fence, since a parked item is explicitly not the one to launch). `--json` emits a versioned v2 envelope — complete `open` and `deferred` arrays (the display cap above is a property of the human surfaces only), plus capped `undeliveredDecisions` / `undeliveredAmends` blocks. Every row always carries `pendingDelivery`, `pendingStatusDelivery`, and `pendingAmendDelivery`: the latter is true only when at least one whole-event-valid amend for a tracked append lacks a canonically equivalent tracked amend. Status and amend delivery remain independent. Version 2 replaced a bare array of open items in iterate-2026-08-01-triage-defer-lifecycle; the later fields are additive and deliberately retain v2 so the existing Command Center consumer stays compatible. The WebUI handoff is to consume the always-present amend boolean and `{count, truncated, ids}` block without overloading the status signal. It is a machine contract, but an unenforced one: the Command Center deep-equals its projection against a **committed snapshot** (`server/src/test/fixtures/triage-union-cli-list.json`) that only a human refreshes via `server/scripts/regen-triage-fixtures.py`. No CI job in either repository re-runs this command, so **any** drift — formatting or a renamed field — is silent until someone regenerates. Treat the contract as a convention, not a gate.
  - `uv run shared/scripts/tools/triage_cli.py show trg-XXXXXXXX [--json]`: fetch one fully resolved card without parsing the whole list.
  - `promote`, `dismiss`, `defer`, `unpark`, `snooze`, and `amend` keep their existing human output by default; adding `--json` emits `{"operation": "…", "item": {…}}` on stdout, including the resolved resulting card state. This is the supported machine-write contract for the Command Center follow-up.
  - Machine failures have stable exit codes: `2` invalid usage/input, `3` status precondition (including a stale CAS decision), `4` unknown item id, `5` uninitialised store, and `6` lock timeout. Error detail remains on stderr.
  - `uv run shared/scripts/tools/triage_cli.py promote trg-XXXXXXXX --task-ref "EXT:linear-ENG-7"`: promote to backlog.
  - `uv run shared/scripts/tools/triage_cli.py dismiss trg-XXXXXXXX --reason notRelevant`: dismiss as false-positive / won't-fix. The human command requires a reason; its Command Center `--json` route also accepts the WebUI's absent/blank reason and records no invented rationale.
  - `uv run shared/scripts/tools/triage_cli.py defer trg-XXXXXXXX --reason "waiting on upstream" --revisit 2026-09-01`: defer — decided, but deliberately not now (`snoozed`). A reason **and** a revisit date are required. From 00:00 UTC on that date the item is back on the open list by itself, with nothing written to the log; until then the same finding is not recorded a second time, and it still closes automatically if the finding disappears. Already-parked items can be deferred again to correct the date.
  - `uv run shared/scripts/tools/triage_cli.py snooze trg-XXXXXXXX [--reason "waiting on upstream"] [--revisit 2026-09-01] --json`: Command Center-compatible snooze. Its reason and date are optional because the existing WebUI route permits both to be omitted; when a date is supplied it must be a future UTC calendar day, so a successful snooze always remains snoozed. It is intentionally distinct from the stricter human `defer` command.
  - `uv run shared/scripts/tools/triage_cli.py unpark trg-XXXXXXXX --reason "parked by mistake"`: reverse a deferral — back on the open list, revisit date cleared. Refused for anything not currently parked (including a park whose date has already passed, which is open again anyway). Never hand-edit `triage.jsonl`.
  - `uv run shared/scripts/tools/triage_cli.py amend trg-XXXXXXXX [--title T] [--detail D] [--severity S] [--kind K]`: correct an open card's title/detail/severity/kind IN PLACE — the id, status, and audit history are untouched; at least one field is required. A stale or already-decided card is refused atomically (exit 3). Use this instead of dismiss-and-refile when a card's *content* was wrong, not its disposition.
...
- **Defer:** `triage_cli.py defer <id> --reason <reason> --revisit YYYY-MM-DD` for work that is real but not now; the item flips to `snoozed`, moves into each surface's deferred section instead of disappearing, and returns to the open list by itself on the named day. `triage_cli.py unpark <id> --reason <reason>` reverses it.
- **Amend:** `triage_cli.py amend <id> [--title T] [--detail D] [--severity S] [--kind K]` corrects a card's content in place — id and disposition unchanged — for when the card itself was wrong, not the decision about it.

Legacy producers (phase-quality, drift, compliance, security, performance, F0.5) stay finding-granular; they MAY emit `launch_payload=None` and the renderer falls back to the historical bullet layout. Only the GitHub action-unit emitter populates `launchPayload` today; the WebUI Triage-tab in `shipwright-webui` is a separate implementation in a separate repository that mirrors these helpers' semantics rather than sharing their code, and the two are known to diverge (it permits a defer with neither a reason nor a revisit date, both of which the CLI requires; such a park resolves as parked-but-never-due rather than being silently re-opened). Treat the CLI as the reference, not as a guarantee that both surfaces validate alike.
...
per Promote / Dismiss / Park / Un-park).
...
| `OPENROUTER_API_KEY` | OpenRouter key for GLM + GPT external review | Plugin (optional) |
| `OPENAI_API_KEY` | Direct OpenAI key for the GPT review arm only | Plugin (optional) |
...
Every other review judges a change **within** the frame its plan set -- does the diff fulfil the spec, is the plan internally sound, do the claims hold. None asks whether the thing should be built. Measured: an iterate that added roughly 5000 lines of delivery machinery passed three review rounds and 25+ findings without one asking that question; the same two models, given a brief instead of a plan, both rejected it and both independently named a simpler alternative the plan had already discarded.

So the architecture pass is **a second call inside the same review step, not a second step** -- and what makes it work is the **input**, not the prompt wording. A plan carries its own `Alternative approach -- rejected because X`; a reviewer handed that document has been handed the answer. This call reads a short **brief** instead (template: `shared/templates/architecture_brief.md`) listing the options **without** the reasons any were rejected. The CLI refuses `--plan-file` in this mode so the anchoring cannot creep back in unnoticed.

**It runs on every medium+ Branch A of a standalone iterate, and on every `/shipwright-plan` Branch A** -- no trigger, no declaration. (Campaign sub-iterates do not run it yet: that runner inlines its own copy of the review step and cannot stop to ask an operator.) That is deliberate: a trigger the author sets fails first on exactly the changes that most need the question asked. The cost stays low because a change that adds nothing permanent gets a **three-line brief** ("Nothing. This changes machinery that already exists: ..."), which the reviewers confirm in a sentence.

**On a `reject`, the run stops and asks you** -- take the alternative, keep the plan with the reason recorded, or rework and re-review. The reconciliation against the plan's own rejection rationale is written down at that point, which is where the withheld reasoning re-enters the record.

**Provider configuration (same for all four modes):**

**Option A -- OpenRouter (recommended):** One key covers both review models. GLM requests are fail-closed to the configured, code-verified zero-retention endpoint allowlist. The shipped policy permits only `novita` and `together` in the US, sends `provider.only` and `provider.order` in that order, disables fallbacks, denies data collection, and requires ZDR. Reasoning effort is capped to `low` to keep GLM from exhausting its output-token budget on invisible reasoning on large diffs. If neither endpoint is available, the GLM arm is reported unavailable/degraded; Shipwright never widens routing or falls back to Gemini.

```
OPENROUTER_API_KEY=sk-or-your-key
```

**Option B -- Direct OpenAI:** Run only the GPT review arm directly.

```
OPENAI_API_KEY=sk-your-key
```

**Option C -- Skip external review, rely on the internal reviewer:** Leave keys unset. `/shipwright-plan` and medium+ `/shipwright-iterate` will prompt you at Step 5. Both now run the same internal reviewer (`opus-plan-reviewer`, model resolved from `plan_review`) before the prompt, so skipping external review still leaves an independent pass on record. For `/shipwright-plan`, the mandatory self-review ("2x denken") pass only runs as a last resort if that internal reviewer itself could not run. For `/shipwright-iterate`, self-review is unconditional at every complexity regardless of the internal pass's outcome -- the internal arm is an additional independent layer, not a trigger or a replacement for it. Either way, the opt-out decision is logged in the decision log / iterate ADR and `external_review_state.json`, so audits still see it. Silent skipping is no longer possible.

**Option D -- Codex CLI instead of a metered GPT key:** If you have a ChatGPT/Codex subscription, the GPT ("openai") reviewer leg can run through the `codex` CLI instead of OpenRouter/direct OpenAI -- flat-cost under your subscription rather than metered per review. Opt in with `external_review.gpt_leg.provider: "codex"` in `shipwright_iterate_config.json` (shared default stays `"api"`, so nothing changes unless you set this). Requires `codex` installed and authenticated (`codex login`); if it's unavailable when a review runs, that pass falls back to Option A/B automatically (and says so in the recorded reason) rather than failing. GLM's leg is unaffected either way -- it has no Codex-CLI equivalent and always answers via OpenRouter.
...
**Diff exposure warning:** Code-review mode transmits the staged diff to whichever LLM provider is configured. Diffs can contain secrets, customer data, or code under restrictive license/NDA terms more often than markdown plans do. If those risks apply to your project, leave the cascade off (build) or set `external_code_review.enabled: false` (iterate). The diff is written to a private, run-scoped scratch path (outside the repo, resolved by `shared/scripts/tools/review_scratch.py` and removed afterward) -- the same file the internal subagent uses. Earlier versions used a fixed `/tmp/shipwright-review-diff.txt`, which bash and native Python resolve to different files on Windows; this file is what closed that.
...
**Model identity lock:** The review arms are fixed to `z-ai/glm-5.3` and `openai/gpt-5.6-terra` (or `gpt-5.6-terra` direct). A differing config value or `SHIPWRIGHT_REVIEW_MODEL_<KEY>` override fails before client creation, preventing a model from travelling under the wrong reviewer identity or routing policy.
...
1. **Quick Estimate** -- A classifier script analyzes your prompt for scope keywords and risk signals, producing an initial estimate with confidence score. When no scope keyword matches, the default is history-calibrated: the median final complexity of your last finalized iterates instead of a blanket "trivial", so the estimate adapts to each project's reality (`signals.prior_source` reports `keyword`, `history`, or `default`). **That default is capped at `small`.** Reaching `medium` — the first level that asks you for a spec, a mini-plan, an approval and an external review — takes a positive signal: a scope word in your request, a risk flag, a change spanning two planning splits, or the repo scan in step 2. Not simply the absence of one. Without the cap the estimate fed on itself: a change with no scope word was called `medium`, that run then became part of the history the next estimate averaged, and the average never came back down — measured on this repo, 84% of the recorded runs sat at `medium`.

**What you give up when a change lands `small` instead of `medium`:** no written spec, no mini-plan, no approval step, no external review, no confidence calibration; tests run for related files rather than the whole suite; end-to-end checks become conditional instead of automatic; and two finalization checks on this list (surface verification, cross-layer coverage) report "skipped" rather than running. **Integration coverage is the one on this list you do not get out of:** if the change touches shared cross-component machinery, that check recomputes from the actual diff and blocks at every level, including `small` and `trivial` (iterate-2026-08-01-coverage-gate-recompute-order). Clearing it does not require escalating — record the one integration behaviour in the run's own entry. Two other checks likewise never stand down by level: the CI supply-chain acknowledgement, and removal coverage, which from the same change reports an **error** rather than a skip when it cannot run at all (no `--commit`, no resolvable base, collector unavailable) — a removal is never trivial, so a check that cannot answer must say so instead of passing quietly. That is the point of the change — but it is a real reduction, not a free one. The safety net is that anything genuinely large still reaches `medium` through a different door: a scope word in your request, a risk flag, a change spanning two planning splits, or step 2's repo scan — which now inspects the actual changed files, so a change to shared machinery is caught even when your wording never mentions it.
...
Two layers enforce it. At finalization `record_event.py` refuses (exit 1) a feature/change iterate that names no FR and gives no `spec_impact=none` justification; and the F11 verifier fails the run if the iterate's own work touched no `spec.md` without a recorded `spec_impact=none` (measured across the iterate's work up to the recorded commit rather than just the tip commit, so an integration merge at HEAD normally does not hide it; where no trunk base can be corroborated the check falls back to the single commit). The on-demand `/shipwright-compliance` detective audit (Group D, check D5) additionally surfaces any historical feature/change events that landed with no FR linkage. BUG iterates are exempt from the spec-impact gate -- a fix need not change the spec.
...
| `touches_build` | JS: `package.json`, `package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, `bun.lockb`, `npm-shrinkwrap.json`, `*.config.*`, `tsconfig.json` · Python: `uv.lock`, `poetry.lock`, `Pipfile`, `Pipfile.lock`, `pyproject.toml`, `setup.py`, `setup.cfg`, `requirements*.txt` | performance test layer (Lighthouse + bundle gate, /shipwright-test Step 3.8). On a Python change that layer mostly skips itself (no dev server → no Lighthouse, no bundle → no size gate); the working effect is the `small` floor plus the full-code-review trigger that any risk flag turns on at trivial/small |
...
Campaigns run **interleaved-serial** on the *merge* lane, and this is the *default* -- not a choice the agent re-decides each run. Within one **wave**, up to a bounded number of ready sub-iterates build **concurrently**, each in its own per-unit worktree, each opening its own PR left open once it finishes; but those PRs are then reviewed and merged **one at a time**, in a fixed order -- the merge lane never runs two merges at once, so there is only ever one sub-iterate PR *being merged* at a time, even though several may sit open together mid-wave. Once every PR from a wave has drained through the merge lane, the *next* wave's ready set is computed off the now-advanced `main` -- waves themselves never overlap. The merge is verified, never armed-and-forgotten: if a PR's CI fails, conflicts, or times out, the campaign strict-stops (already-merged sub-iterates stay durable) rather than charging ahead.

The merge lane is serial rather than "build all the PRs first, then merge them at the end" in order to avoid **merge theater**. Every sub-iterate regenerates the same *derived* artifacts -- the event log, triage, the compliance reports, the dashboard -- and sibling sub-iterates often edit the same shared source files. If they are all built before any merge, none of them sees the others, so every merge has to 3-way-resolve and regenerate those snapshots against an advancing `main`. Interleaved-serial merging sidesteps that ACROSS waves: because the next wave starts from a `main` that already contains every merge the drained wave just performed, shared-file and snapshot edits compose naturally there -- no end-stage drain and no regenerate-at-merge step between waves. **Known gap, WITHIN one wave:** two sibling units that both branched from the same pre-wave `main` and both touch a shared derived file can still hit that same 3-way problem once the later one merges -- narrowed to within a wave, not eliminated, and closed by a later sub-iterate (the serial merge lane's own review-pinning and staleness-cascade revision), not by this one.
...
0. **Fresh verification (F0)** -- run the full canonical unit/type/integration suite; sibling worktrees share weighted host CPU and uv warm-up leases, and a measured per-run request permits compatible siblings to overlap without oversubscription. The runner emits bounded queued/start/completion records and heartbeats, reaps the complete owned process tree on timeout/cancellation, and retains a redacted, untrusted failure tail under the canonical main repository's gitignored run directory. Linked worktrees resolve that stable store before writing, so a tracked race-card link does not disappear when its delivery worktree is removed. Diagnostic file shards are allowed only after an interrupted run and never count as F0 completion; the full canonical runner must still exit zero. Any non-zero exit means STOP, so do not enter F0.5.
0.5. **End-to-End Verification (F0.5; medium+)** -- run the surface runner (`web` / `cli` / `api`, or `none` with justification) against the user-erlebbare Surface via `shared/scripts/surface_verification.py`. A framework iterate changing the F0 host-resource lease must use `suite_host_resources.py --probe` as its recorded CLI runner; it acquires/releases uv and CPU leases, and non-zero is STOP. Produces the `surface_verification` block in `shipwright_test_results.json.iterate_latest`. Non-zero exit blocks the rest of finalization; the post-commit audit `check_surface_verification` in `iterate_checks.py` is the second layer.
...
3. **ADR (decision-drop)** -- record the decision via `write_decision_drop.py`. Each field (`--context`, `--decision`, `--consequences`, `--rationale`, `--rejected`) is hard-rejected at write time if it exceeds 500 characters (`decision_log.md` is always-loaded Layer-1 context and every verbose ADR pays for itself in tokens on every future run). Move long-form prose into the flat ADR spec folder `.shipwright/planning/adr/<run_id_sanitized>-<slug>.md` (named by run_id, never a hand-guessed number) and link it with `--spec-ref`; the aggregator emits a `**Details:** [...]` bullet under the ADR row. Writing the drop also refreshes `.shipwright/planning/adr/INDEX.md`, so the index row ships in the same commit as the ADR file (F6 stages the folder); the release pass refreshes it too. `--spec-ref` is also required whenever a decision needs a diagram, >3 alternatives, or non-trivial follow-up acceptance criteria.
...
12. **Merge, push & verify** -- after F11's `ensure_current.py` refresh has integrated and regenerated, run the guarded `verify_local.py` CI-gate mirror on that actual pre-push tree. Then, if `OPENROUTER_API_KEY` is set, run the **local PR-review preflight** (`pr_review.py --base <default>`) against the still-unpushed branch's merge-base diff -- the same prompts/model/filter the required CI Tier-3 gate uses, catching that gate's threat-model findings before the push+CI round trip rather than after. It is advisory infrastructure, never a substitute: a genuine BLOCK stops here and must be fixed, but the preflight's own unavailability (no key, transport failure) only logs and continues -- it cannot post a PR comment, review state, or satisfy the CI gate's own record. Then push branch and open PR via `gh pr create`. This can be an accepted **late STOP** after F6 committed: F0 remains the cheap early check, but it cannot see finalization writes or the integration. Arming auto-merge is no longer a step of its own: it is the first rung of the delivery ladder in step 13, because whether anything will *ever* merge this PR is only answerable from how the arm turned out. Then verify event was recorded, regenerate session handoff via the **runtime/snapshot split**: Stop hooks write live state to `.shipwright/agent_docs/runtime/` (gitignored); iterate-finalize is the **sole producer** of the tracked `session_handoff.md`, `build_dashboard.md`, and `triage_inbox.md`. The repair pass refuses to run when the session→worktree pointer is invalid; stale-pointer sessions no longer poison main.
13. **Deliver it** -- arming auto-merge is not the same as having delivered. A Required Check can fail *after* the arm, leaving the PR sitting un-merged and red while the run walks away: shoot-and-forget. So finalization's last step runs `shared/scripts/tools/deliver_pr.py`, which holds one bar -- *delivered = the PR is merged and every check that exists is green* -- and decides **who merges** from what the code host can actually do. It arms auto-merge (`iterate/*` only; a manual human PR never self-arms). If the host refuses the arm, it asks why from two facts it can read without admin rights -- whether the repo allows auto-merge, and whether the target branch is protected. Either one missing means the host can *never* arm it, so Shipwright waits for green, brings the branch up to date, **re-runs the guarded local mirror before the refresh push**, re-checks it and merges the PR itself, pinned to the exact commit it verified. A red mirror is the same accepted late STOP and the refresh is not pushed. Any other refusal -- a draft, a conflict, a fact it could not read -- is treated as temporary and it keeps watching, exactly as before. While the PR is still open it reports **why it is not merging** -- conversations still unresolved, required checks that never reported at all, and the code host's own verdict that the merge is blocked -- rather than only how long it has waited; anything it could not check is reported as unchecked, never counted as clear. Exit codes: `0` merged, `2` a Required Check failed, `3` closed, `4` still pending when the watch timed out, `5` the host could not be read, `6` nothing can merge it and Shipwright may not (`SHIPWRIGHT_ITERATE_SELF_MERGE=0`) -- reported at once rather than waited on, because re-running changes nothing -- and `7` delivery refused (the re-check came back red, the PR was not this run's, or the host refused the merge). Only `0` means delivered, and the report always names who merged it and how many checks the host ran.
...
2. **Subagent code review** -- A dedicated `spec-reviewer` -> `code-reviewer` -> `doubt-reviewer` cascade examines the diff against the spec and flags issues. Triggers on diffs > 100 lines, security-sensitive files, or medium+ complexity iterates. **A standalone iterate spawns this cascade itself** (iterate `SKILL.md` Step 8), before the commit. A project whose `CLAUDE.md` carries the **standing-request** section spawns the cascade without asking — that section is the request. Without it, the iterate **asks you for the go-ahead before it starts reviewing** rather than quietly recording the pass as "not run" at the end; such policies are conditional, and one sentence lifts them for the rest of the session. Dynamic workflows and deep-research are never covered by that grant and are asked for every time. If you decline, the run records that in the review record and the external review carries the pass. Under `--autonomous` it cannot pause to ask — so *unless the session already holds permission*, in which case the cascade runs normally, it records the gap honestly and names it in the closing run summary. The campaign sub-iterate-runner never can -- it is a subagent without the `Agent` tool, so its cascade is delegated to the orchestrator (ADR-029, campaign mode only), which runs it at campaign-loop step `3f-bis`: after the sub-iterate is built and recorded, but **before its PR is merged**, so a spec-compliance REJECT can still stop delivery. The runner writes those rows `not_run` -- true when it writes them -- and `3f-bis` promotes them once the passes have actually run, so the record names whoever performed each one. Read that delegation as campaign-specific: taken as a general rule it tells every iterate not to spawn reviewers, which is exactly how runs reached `main` with no internal review. Each of the three stages closes its own row in the run's review record, so a finished run can show *which* reviews happened rather than asserting that they did: recording the code review as done while the spec-compliance stage never passed is refused, and a "done" row that carries no trace of an actual review — no findings, no provider, no reviewer name — no longer counts as one.
3. **External LLM code review (cascade)** -- Optional second-opinion gate that sends the same diff to ZDR-routed GLM + OpenAI in parallel against the section/iterate spec. Build: opt-in via `shipwright_build_config.json` -> `external_code_review.enabled: true`, default off. Iterate: default-on for medium+ runs on its own thresholds, with Branch A/B/C interactive opt-out (same flow as the mini-plan review). See `External LLM Review` in Chapter 6 for provider setup and the diff-exposure caveat. **It does not substitute for layer 2:** the external route is a generic code-quality opinion, so the Stage-1 spec-compliance HARD-GATE and the Stage-3 adversarial pass have no external counterpart. When the F11 floor is carried by the external review alone, the gate says so in its own output.
...
**Which version directory gets compared — the sync's answer, not a second opinion.** `update-marketplace.sh` copies each plugin into the directory named by `~/.claude/plugins/installed_plugins.json`, so the check reads that same file rather than deciding for itself. It used to take the highest SemVer directory present, which agrees only because the sync's last step deletes every version dir that is not the installed one — let one survive (a `rm -rf` that lost to a Windows file lock, a `claude plugin install` that materialised the repo's newer version, an aborted run) and the two halves describe different trees: the sync reports "up to date" while `--strict` reports drift with every repo file missing. Measured 2026-08-01, all 14 plugins carried repo version 0.31.0 against installed 0.2.x, so a survivor always sorts *above* the live directory. When the manifest cannot be read the highest version is still used, but the run says so — see `version_basis` below. This is the same defect class as the `.python-version` exclusion the comparator had to be taught: one contract, two implementations, pinned to each other by `shared/tests/test_plugin_cache_version_pin.py`.

The cache carries three trees, and all three are gated — a green means all were read:

| Cache tree | Reached at runtime as | Holds | Gated |
|---|---|---|---|
| `<plugin>/<version>/` | `${CLAUDE_PLUGIN_ROOT}` | each installed plugin (the version dir `installed_plugins.json` names) | yes |
| `shared/` | `${CLAUDE_PLUGIN_ROOT}/../../shared` | the shared tools every phase runs, including the F11 finalization verifier | yes |
| `plugins/<plugin>/` | `${CLAUDE_PLUGIN_ROOT}/../../plugins/shipwright-X` | the cross-plugin mirror (a symlink on POSIX, a real copy on Windows) | yes — own basis (see below) |

The mirror joined last (P2.29), once the SessionStart self-heal hook (`ensure_shared_cache.py`) started comparing each mirror's file set against its own repair source instead of one whole-tree sentinel — a gate built on the older sentinel-based healer would have inherited its blind spot. Its basis is always the literal string `"cache"`, never `git` or a `walk (<why not git>)` fallback: its repair source is itself a prior sync's cache-side copy — the plugin's own newest cached version, never `installed_plugins.json` (the healer doesn't consult that manifest either) — so there is no git-vs-clone line-ending seam to name. `scripts/cache_mirror_compare.py` holds the primitives; the healer still answers a different question (ABSENCE, via a file-set diff) than this check does (STALENESS, via the same CRLF-normalized content hashes as the other two trees).

**Reading the verdict.** `--json` carries `verified` (which trees this result actually covers), `ungated` (which it knowingly does not — empty now that all three trees are gated, kept in the schema for stability), per record a `basis` of `git`, `walk (<why not git>)`, or `cache` (the mirror's own), and per plugin a `version_basis` of `installed_plugins` or `latest (<why the manifest was not used>)`. The `version_basis` note is printed on the `ok` line as well as the WARN line, because a green over a guessed directory is the more dangerous half: a fresh higher version dir that happens to match the repo reads as "in sync" while the tree runtime actually loads is the stale installed one. `status: ok` alone reads the same whether `shared/` was compared or skipped — that ambiguity was the original bug, and a machine consumer needs to be able to tell. The human lines say the same: the `ok` line now names the mirror's own coverage (plugin count and file count), and both the `ok` and the WARN line name any `plugins`/`shared` basis that is not `git` — the mirror's permanent `cache` basis is deliberately kept out of that note, since it is not a degraded fallback. That basis note matters on a drift: if git refuses (`safe.directory` on a UNC path or a container uid) the repo side reverts to walking, the gitignored generated files reappear as phantom missing-from-cache, and the printed remedy would send you re-syncing a cache that was never the problem. A `plugins/` directory that exists but cannot be listed is its own status rather than a green over zero plugins.
...
uv run scripts/check_plugin_cache_sync.py --installed-plugins <path>   # non-default manifest
```

`--installed-plugins` defaults to the real manifest only when `--cache-root` is also the real one: it describes *that* cache, so pointing the check at another tree without naming a manifest falls back to the highest cached version rather than silently reading this machine's state.
...
| **C3** | `.shipwright/agent_docs/session_handoff.md` canon-marker frontmatter names THIS phase and its latest recorded completion, and is not older than it. When another phase owns the note, the two phases are ordered against each other: the later one owes a note, the earlier one is a named skip | WARNING |
...
> **`--commit` is required for the traceability gates.** The two enforcing F11 gates (`removal_coverage`, `cross_layer_coverage`) fail-closed: they need `--commit` to diff base→head, so a commit-less `verify_iterate_finalization.py` (or `verify_phase.py --phase iterate`) reports `cannot enforce … no --commit` as an **error**, not a green skip. `removal_coverage` does this at **every** complexity (a removal is never trivial, and gating the refusal on the run's own self-reported complexity label was the defect — iterate-2026-08-01-coverage-gate-recompute-order); `cross_layer_coverage` runs at medium+ only, by a deliberate cost decision about scope. `removal_coverage` establishes the git context first, so a project that is not a git repository skips rather than erroring on a commit it was never going to have; `cross_layer_coverage` keeps the original order and reports the missing `--commit` first, so on a non-git project at medium+ it errors. This is a diagnostic behavior change — a commit-less error is not a real gate failure; pass `--commit $(git rev-parse HEAD)`. The pipeline/SKILL invocations already do.

> **`cross_layer_coverage` reads acceptance criteria, not just the FR row.** It decides "did behaviour change?" from the difference between the base and head specs. Comparing only the requirement's *table row* (name, layers) made it blind to the authoring pattern [`shared/fr-authoring.md` §3](../shared/fr-authoring.md) actually recommends — **folding**: recording a change as a new acceptance criterion on an existing requirement instead of adding a row. A folded change left the row untouched, so the gate always answered "could not determine". It now also compares each requirement's own acceptance criteria, so a folded change is attributed to the requirement it belongs to and checked at that requirement's layers. A spec edit that touches neither a row nor any criteria — an abstract, a quality requirement, prose between sections — is still reported as could-not-determine for a human to judge, never quietly passed.

> **The keystone gate: a changed acceptance criterion must have re-run its tests, green.** `cross_layer_coverage` above answers *which requirement's layers a change should be tested at*. The **Keystone AC gate** answers the sharper question one level down, and it runs in **CI on every pull request**, not at F11: *this PR edited acceptance criterion `FR-01.11/AC17` — did the tests bound to **that criterion** actually run, in this very CI job, and pass?* It reads the criteria out of `spec.md` itself (each carries a permanent `[ACnn]` marker), diffs them against the merge base, and then requires **every** test bound to a changed criterion to have run and passed — not merely one of them. Three consequences worth knowing before you meet it: a PR that changes code but no acceptance criterion passes untouched (it enforces spec-to-test consistency, not code-to-spec consistency); editing prose, a heading, or a README never fires it; and editing a criterion while quietly deleting the `@covers("FR-xx/ACnn")` tag that bound it to a test is a **hard block**, because that is the same thing as deleting the check. Criteria with no test yet are **reported, not blocked** — the gate protects what is bound and grows as binding does. It runs after the traceability-manifest regeneration step for a reason: that step rewrites the evidence from the run's own test output first, so a hand-edited manifest claiming green cannot change the verdict.

> **Two feeder checks, deliberately asymmetric.** Alongside the keystone gate, two more CI-only checks close the same job. `AC coverage ratchet` answers *"AC without a test"* — this is **anti-ratcheted**, not hard: a real legacy backlog exists (most minted acceptance criteria have no test bound to them today), so it only blocks a *newly* unbound criterion that isn't already in the grandfathered baseline (`shipwright_ac_coverage_baseline.json`), never the pre-existing backlog. `Orphan AC binding` answers the opposite case — *"a test whose AC vanished"* — and is **hard from day one, with no baseline at all**, because nothing has ever checked whether a test's binding still points at a real criterion, so there is no backlog to forgive. It catches an outright-deleted or renumbered criterion that still has a test pointing at it, and — separately — a binding quietly dropped in a PR that never touches the criterion's own wording, detecting that first PR of a two-step way to defeat the keystone gate (drop the binding in one PR, edit the now-unbound criterion in a later one) whenever the base commit's own manifest record of the binding is trustworthy — not unconditionally, since that arm's only evidence is a stale-by-construction base artifact.

> **A third check, advisory-only, never a gate.** `Check-body suspects` flags an acceptance criterion whose own text is unchanged, that has a test bound to it whose *body* was edited in the same PR — a co-occurrence worth a human's attention (was the assertion refactored, or quietly weakened?), never a verdict the check renders itself. Its own CLI returns success unconditionally: a clean run, a flagged suspect, and an infrastructure fault are all reported in its JSON output, never through the process exit code, so it can never become a merge blocker even by accident.
...
| `decision_log.md` | Architecture Decision Records (ADR format). Compact one-row entries with mandatory 500-char field caps; long-form prose lives in the flat `.shipwright/planning/adr/<run_id_sanitized>-<slug>.md` spec folder | All phases |
...
| `iterates/<run_id>.json` | Compact per-iterate record (one file per iterate; legacy `iterate_history` array is migration-only) | Iterate finalization (F5c, 200-entry retention) |
...
**ADR spec folder:** `.shipwright/planning/adr/<run_id_sanitized>-<slug>.md`, flat, one file per ADR, named by `run_id` rather than a hand-guessed number — `run_id` is already globally unique, so nothing to coordinate across parallel iterates (a numeric `<NNN>-<slug>.md` convention preceded this and let branch-time guesses collide; existing files in that shape are grandfathered and reported, not renamed, by `iterate-2026-08-08-index-readers-adr-lock`). Drops opt in via `--spec-ref <path>`; aggregation renders a `**Details:** [filename](../planning/adr/...)` link. `INDEX.md` beside them is generated, never hand-edited — each row's label is that ADR file's own first `#` heading, so retitle the ADR rather than the index. It is refreshed both when a drop is written (iterate F3) and on every release pass; regenerate by hand with `uv run {shared_root}/scripts/tools/rebuild_adr_index.py --project-root .`. Schema lives at [shared/schemas/decision_drop.schema.json](../shared/schemas/decision_drop.schema.json).
...
Two different answers, and the distinction matters if you are relying on them.

**In your working tree:** current. Compliance is regenerated incrementally after each pipeline phase and at every iterate finalization, so what you see locally reflects the state at that moment.

**Committed on the default branch:** current **as of the last release or documents-only refresh** — not continuously. That is a deliberate choice, and the seven files say which state they describe: each markdown document's `Source-State:` line carries `base=<commit>` (what it was computed from) and, when it shipped with a release, `release=<tag>`. For audit evidence a fixed point is a better claim than a moving one.

**Why not continuously?** Because a branch cannot compute them correctly. These documents are *derived* — every line is recomputable from `shipwright_events.jsonl`, `.shipwright/triage.jsonl` and git history, all of which already ship — but a branch-local derivation reads the *branch's* git history and an event log missing every branch merging alongside it. Measured on 2026-07-27: the default branch's committed change history over-counted commits by 11 and named a commit that had never been on it. Twenty-five parallel branches also all rewrite the same seven files regardless of what they changed, producing conflicts in series. So iterate branches do not carry them (`check_no_derived_snapshots_committed` enforces it), and the refresh happens where one branch, one human and one reviewed pull request make it correct.

**To refresh them between releases**, run `/shipwright-compliance --refresh-pr`. It recomputes the documents and opens an ordinary documents-only pull request — same checks, same visibility as any other change, under your own GitHub login. There is no bot with write access to the default branch, no deploy key, and no exception to the branch-protection rule anywhere in this path.

The moment you need current evidence is the moment to run it. The rest of the time a fixed, honestly-labelled point is enough.

**One document nearby is NOT in the set, and does not get refreshed at all.** `.shipwright/agent_docs/triage_inbox.md` is tree-derived and would be perfectly refreshable, but it lives outside the compliance directory and the release phase does not recompute it. Before this change all eight tree-derived documents were equally frozen, which was at least one rule; now seven are refreshed and stamped and that one is not, and nothing on its face says so. Read it as a view of `.shipwright/triage.jsonl` at some past point, and read the backlog itself (or the Command Center) when you need the current one.

**One exception inside the set.** `ci-security.json` is derived from the **latest completed CI security scan**, not from the repository, so its freshness is not a property of the commit it ships in. It states its own `source` and `scan_date`, and the refresh reports when that scan predates the code beside it. It never blocks a release: a release is not held for a scan that has not landed.
...
| External review prompt appears every run | No API key set for OpenRouter or OpenAI | The skill prompts in Step 5 Branch B rather than silently skipping. Fix: add `OPENROUTER_API_KEY=...` to `.env.local` at the repo root. To permanently opt out, set `external_review.feedback_iterations: 0` in `shipwright_iterate_config.json` (the only file the resolver reads for this setting -- a `shipwright_plan_config.json` copy of the key has no reader). Both `/shipwright-plan` and medium+ `/shipwright-iterate` then rely on the internal reviewer (`opus-plan-reviewer`), which already ran and carries the gate. `/shipwright-plan`'s self-review fallback only applies if that internal pass itself could not run; `/shipwright-iterate`'s self-review always runs regardless, at every complexity. |
...
| `/shipwright-iterate` | `"description"` | `--type feature\|change\|bug`, `--complexity trivial\|small\|medium\|large`, `--review`, `--pause`, `--campaign <slug>`, `--sub-iterate-id <id>`, `--autonomous`, `--review-model opus\|sonnet\|haiku\|inherit\|fable`, `--finalization-model opus\|sonnet\|haiku\|inherit\|fable`, `--plan-review-model opus\|sonnet\|haiku\|inherit\|fable` | Complexity-adaptive SDLC for ongoing changes. Auto-detects intent and complexity, scales phases from quick fix to structured mini-pipeline with planning, review, and testing. Campaign mode (`--campaign`) groups related sub-iterates; `--autonomous` runs a bounded WAVE of ready sub-iterates **concurrently** -- each its own PR, opened and left open -- and merges them one at a time through a serial **merge lane**, in fixed order, before computing the next wave's ready set off the now-advanced `origin/main` (`branch_strategy: serial`, the default), so shared-file edits compose ACROSS waves with no end-stage merge drain. `--campaign <slug> --sub-iterate-id <id>` on a single hand-run sub-iterate stamps `campaign`/`sub_iterate_id` into its `work_completed` event (same self-identification the autonomous runner records). `--review-model`/`--finalization-model`/`--plan-review-model` override the Claude model tier the review cascade / `sub-iterate-runner` spawn / medium+ Internal Plan Review sub-step (before Branch A/B/C, over the iterate spec + mini-plan) run on, for this run only. With no flag, the role falls back to whatever `shipwright_model_config.json` at the MAIN repo root (worktree-aware — a linked worktree's own copy is never read) sets for that role; with neither a flag nor a config entry, it defaults to `inherit` (today's behavior: follows the session's own model). |
...
| `/shipwright-plan` | `@spec.md` | -- | Create implementation plan for one split. Researches stack, interviews for clarification, generates section files. Always reviews the plan with an internal reviewer first (model tier resolved from the project's `shipwright_model_config.json` `plan_review` role — no invocation flag of its own, since `/shipwright-plan` takes no flags at all; unset resolves to `inherit`, i.e. the session's own model, same as the other three roles); optionally sends it to external LLMs (GLM + OpenAI) for review on top of that. |
| `/shipwright-build` | `@section.md` | `--autonomous`, `--from <section>`, `--review-model opus\|sonnet\|haiku\|inherit\|fable`, `--finalization-model opus\|sonnet\|haiku\|inherit\|fable`, `--execution-model opus\|sonnet\|haiku\|inherit\|fable` | Implement one section using TDD. Writes failing test, implements code, runs code review subagent, creates Conventional Commit on feature branch. With `--autonomous`, loops through all pending sections via subagents without manual gates. The three `--*-model` flags override the Claude model tier the review cascade / `section-builder` / `browser-fixer` spawn on, for this run only (same resolver `/shipwright-iterate` uses — see its entry above); unset defaults to `inherit`. |
...
| `generate_session_handoff.py` | Session-handoff writer. The `--canon-marker` flag lets phase finalization steps tag the handoff so the PostStop hook does not clobber it. | `--canon-marker` (emit YAML frontmatter with `canon_generated: true` + `run_id`) · `--phase <phase>` · `--preserve-canon-marker` (for a write that does NOT pass `--canon-marker`: keep the marker already on disk instead of dropping it, so a mid-phase handoff cannot erase a completed split's C3 evidence; ignored when `--canon-marker` is passed, including its degraded no-`SHIPWRIGHT_RUN_ID` path) · `--reason "..."` · `--project-root <path>`. Requires `SHIPWRIGHT_RUN_ID` env var when `--canon-marker` is set (degrades safely with a stderr warning otherwise). |
| `external_review.py` | External LLM review CLI. Runs GLM + a second identity (OpenAI, or Opus 5 under `--driver codex`) in parallel via OpenRouter (or GPT alone via direct OpenAI) and emits a unified `{success, provider, reviews, verdicts, statuses, contradiction}` JSON envelope. Each reviewer ends with a `SHIPWRIGHT_VERDICT:` line; the envelope carries both verdicts and whether they contradict each other, so a disagreement cannot be flattened into a finding count. Four modes share the same dispatch surface, each bound to exactly ONE input flag — a flag belonging to another mode is a usage error, never silently ignored, because `--mode architecture` earns its different answer only from reading a brief rather than the plan. The GPT ("openai") leg's route is resolved separately from `external_review.gpt_leg.provider` — `"api"` (default, this env-key chain) or `"codex"` (the Codex CLI, flat-cost under a subscription; falls back to `"api"` when unavailable) — that leg only exists under `--driver claude`. | `--mode plan\|iterate\|code\|architecture` · `--driver claude\|codex` (required, no default — `claude` for Claude Code plugins, `codex` when Codex CLI drives the run) · `--plan-file <path>` (plan/iterate) OR `--diff-file <path>` (code) OR `--brief-file <path>` (architecture) · `--spec-file <path>` · `--plugin-root <path>` · env: `OPENROUTER_API_KEY` or `OPENAI_API_KEY` · reviewer/model identities are fixed; differing `SHIPWRIGHT_REVIEW_MODEL_<KEY>` values fail closed |
| `mark-review-state.py` | Writes the marker file that downstream phases and compliance read to confirm a review-step branch (A/B/C) ran to completion. Two filenames: `external_review_state.json` for plan/iterate, `external_code_review_state.json` for the code-review cascade. Markers use schema 4 with `glm`/`openai`, or schema 5 with `glm`/`opus` (the `--driver codex` roster); readers retain compatibility with historical schema-3 `deepseek`/`openai` markers, historical schema-2 `gemini`/`openai` markers, and older unversioned markers. The external-review JSON envelope separately emits `review_schema: 2`; its historical schema 1 was implicit. | `--planning-dir <path>` · `--status completed\|skipped_user_opt_out\|skipped_config_disabled` · `--review-type plan\|iterate\|code` (omitted = plan/iterate marker) · `--provider openrouter\|openai` · `--findings-count N` · `--reason "..."` · `--self-review-fallback-ran` · `--verdict reviewer=approve\|revise\|reject\|unknown\|unavailable` (repeatable, `glm`/`openai` or `glm`/`opus`; the contradiction between the two is derived, never asserted) · `--contradiction-resolution "..."` (required before a disagreement stops blocking) |
