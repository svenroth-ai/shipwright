...
Shipwright can send artifacts to external LLMs (DeepSeek V4 Pro + GPT-5.6 Terra in parallel) for an independent second opinion. The same CLI -- `shared/scripts/tools/external_review.py` -- runs three review modes; which one fires depends on the phase that triggers it.
...
| `external_review.py` | External LLM review CLI. Runs DeepSeek + OpenAI in parallel via OpenRouter (or GPT alone via direct OpenAI) and emits a unified `{success, provider, reviews, verdicts, statuses, contradiction}` JSON envelope. Each reviewer ends with a `SHIPWRIGHT_VERDICT:` line; the envelope carries both verdicts and whether they contradict each other, so a disagreement cannot be flattened into a finding count. Three modes share the same dispatch surface. | `--mode plan\|iterate\|code` · `--plan-file <path>` (plan/iterate) OR `--diff-file <path>` (code) · `--spec-file <path>` · `--plugin-root <path>` · env: `OPENROUTER_API_KEY` or `OPENAI_API_KEY` · reviewer/model identities are fixed; differing `SHIPWRIGHT_REVIEW_MODEL_<KEY>` values fail closed |
