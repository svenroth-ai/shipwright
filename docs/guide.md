...
**What you give up when a change lands `small` instead of `medium`:** no written spec, no mini-plan, no approval step, no external review, no confidence calibration; tests run for related files rather than the whole suite; end-to-end checks become conditional instead of automatic; and several finalization checks (surface verification, integration coverage, cross-layer coverage) report "skipped" rather than running. That is the point of the change — but it is a real reduction, not a free one. The safety net is that anything genuinely large still reaches `medium` through a different door: a scope word in your request, a risk flag, a change spanning two planning splits, or step 2's repo scan — which now inspects the actual changed files, so a change to shared machinery is caught even when your wording never mentions it.
