...
**What it needs.** A `spec.md` file generated by `/shipwright-project`. Recommended: `OPENROUTER_API_KEY` in `.env.local` at the repo root (one key covers both review models). A direct `OPENAI_API_KEY` can run the GPT arm only; DeepSeek always requires the approved OpenRouter ZDR route. If no key is set, the skill will prompt you at Step 5.
...
| `OPENROUTER_API_KEY` | OpenRouter key for DeepSeek + GPT external review | Plugin (optional) |
...
Shipwright can send artifacts to external LLMs (DeepSeek V4 Pro + GPT-5.6 Terra in parallel) for an independent second opinion. The same CLI -- `shared/scripts/tools/external_review.py` -- runs four review modes; which one fires depends on the phase that triggers it.
...
**Option A -- OpenRouter (recommended):** One key covers both review models. DeepSeek requests are fail-closed to the configured, code-verified zero-retention endpoint allowlist. The shipped policy permits only `novita` and `together` in the US, sends `provider.only` and `provider.order` in that order, disables fallbacks, denies data collection, and requires ZDR. If neither endpoint is available, the DeepSeek arm is reported unavailable/degraded; Shipwright never widens routing or falls back to Gemini.
...
**Model identity lock:** The review arms are fixed to `deepseek/deepseek-v4-pro` and `openai/gpt-5.6-terra` (or `gpt-5.6-terra` direct). A differing config value or `SHIPWRIGHT_REVIEW_MODEL_<KEY>` override fails before client creation, preventing a model from travelling under the wrong reviewer identity or routing policy.

**Routing probe:** With credentials available, verify the live DeepSeek route without sending project content:

```bash
uv run shared/scripts/tools/probe_deepseek_zdr.py --project-root . --run-id <iterate-run-id>
```

The probe sends only a fixed synthetic prompt and writes sanitized, run-specific evidence. Missing credentials produce an explicit skipped result; an unavailable or unapproved endpoint produces degraded evidence.
...
| `/shipwright-plan` | `@spec.md` | -- | Create implementation plan for one split. Researches stack, interviews for clarification, generates section files. Always reviews the plan with an internal reviewer first (model tier resolved from the project's `shipwright_model_config.json` `plan_review` role — no invocation flag of its own, since `/shipwright-plan` takes no flags at all; unset resolves to `inherit`, i.e. the session's own model, same as the other three roles); optionally sends it to external LLMs (DeepSeek + OpenAI) for review on top of that. |
