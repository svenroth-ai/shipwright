...
Shipwright can send artifacts to external LLMs (GLM 5.3 + GPT-5.6 Terra in parallel) for an independent second opinion. The same CLI -- `shared/scripts/tools/external_review.py` -- runs four review modes; which one fires depends on the phase that triggers it.
