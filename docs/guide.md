...
- **If the two reviewers disagree about the plan as a whole** -- one endorsing it while the other says the approach is wrong -- that is recorded as its own outcome and put to you to decide. Splitting the plan into sections refuses to start until your decision is on record. The same applies when a reviewer's conclusion could not be read: an unreadable answer is not agreement. Two reviewers exist so a disagreement gets noticed, which a single count of findings cannot show
