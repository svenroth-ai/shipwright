...
**F0 and F11 run this.** F0 runs it right after the leak-guard and before the test
suite; F11 runs it again after `ensure_current` has integrated and regenerated, before
push. Both steps grep this docstring for the marker token
``SHIPWRIGHT_MIRRORED_MERGE_GATES`` rather than merely testing that a file exists at
this path, because `scripts/verify_local.py` is not a distinctive name and a consumer
project carrying one must not have it executed under STOP semantics. **Do not remove
that token**; without it the steps silently stop running here.

F0 remains the cheap early catch. Eight phases write tracked artifacts after it
(F0.5/F3/F3a/F4/F5/F5a/F5b/F5c), and F11 can merge `origin/main` before the push. F11
therefore re-checks every resulting pre-push tree, including a delivery-refresh push.
Its failure is an accepted **late STOP** after F6 committed, because delivery must not
push a tree CI will reject.
That F0 placement was the operator's decision on trg-486cb11c;
