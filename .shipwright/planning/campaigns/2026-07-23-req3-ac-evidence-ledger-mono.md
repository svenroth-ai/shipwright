...
| # | Criterion | Status | Mechanism / gap |
|---|---|---|---|
| 1 | refuses on failing tests until a person confirms | `enforced, tested` | **fixed 2026-09-15 (req3-06 e4).** `validate-deploy.py --confirm-failing-tests` reads `shipwright_test_results.json` (`unit.status`/`e2e.status`, the same fields SKILL.md Step B4 already documented) and refuses (`success: false`, exit 1) unless the flag is set — set only by the SKILL.md after its own `AskUserQuestion`. Reads either of the file's two producer shapes (top-level, or `/shipwright-iterate`'s `iterate_latest`-nested); E2E is non-blocking (`not_run`/`skipped`/absent never refuse) except on a reported `partial` — an F0 real-suite run against THIS repo's own file caught both gaps a tmp_path-only unit test could not. **Round 3 (Tier-3 PR review):** a missing `shipwright_test_results.json` (`test_gate == "no-results"`) also refuses without confirmation — an earlier version of this round warned-and-proceeded on that case, silently loosening the pre-existing (pre-mechanisation) SKILL.md requirement that "tests failed OR file does not exist" both needed a person's confirmation. Tests: `test_validate_deploy.py::test_test_gate_*` |
