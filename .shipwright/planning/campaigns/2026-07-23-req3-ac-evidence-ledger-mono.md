...
| 4 | Basis recorded, and `assumed` does not appear (greenfield) | `enforced, tested` | **Closed iterate-2026-09-11-e2-checks-project-elicitation, merged with #15 (see #15's own note for the resolution).** `_project_gate_extras.basis_forbids_assumed` (new, shared) greps every active FR row's Basis cell across every `spec.md` this project has written and fails on a bare `assumed` OR a qualified `assumed: <reason>` cell (`fr_basis` reports the latter as `malformed`, not `known`/`assumed` — closed as a qualifier-smuggling gap in external code review round 3); wired into `project_checks.run_project_checks` (`check_basis_forbids_assumed`, FR-01.02 #4/#15), the same code-level dispatcher `update-step --step project` already blocks on. **Greenfield-only, per the row's own name** — skipped for `scope: "extension"` (added round 5, both external code reviewers independently: the wiring originally ran unconditionally, inconsistent with #11's own scope carve-out for the same reason). Tested: `shared/tests/test_project_gate_extras.py`, `shared/tests/test_verifiers_project.py`. |
