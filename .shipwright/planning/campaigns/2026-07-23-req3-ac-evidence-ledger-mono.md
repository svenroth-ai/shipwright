...
| 15 | the unconfirmed-assumption basis is allowed only **with what would settle it** named | `enforced, tested` | **Closed 2026-09-11 (e2-checks-project-elicitation), merged into #4.** The 2026-07-25 retro-pass note below ("Both scenarios hit a contradiction the phase carries with itself") is resolved here: rather than build a second, semantic "does this AC actually name a settlement" oracle — no deterministic aboutness check exists, the campaign's own abort condition — #4's already-decided, already-shipped-at-the-grill-trace-layer absolute ban (P4.2's `check_greenfield_assumed` — "no exceptions in this surface") is extended to the FR-row layer instead of layering a second, softer rule on top of it. With `assumed` banned outright, #15's "only with a settlement" is vacuously satisfied — there is never an un-settled `assumed` to catch, because there is never an `assumed` at all. Same mechanism, same tests as #4: `_project_gate_extras.basis_forbids_assumed`, `check_basis_forbids_assumed`. |
