...
| 15 | the unconfirmed-assumption basis is allowed only **with what would settle it** named | `enforced, tested` | **Closed 2026-09-11 (e2-checks-project-elicitation), merged into #4; revised same day post-merge (Stage-1 spec-review REJECT on PR #729).** The 2026-07-25 retro-pass note below ("Both scenarios hit a contradiction the phase carries with itself") is resolved here — but the first cut over-resolved it: it copied #4's already-shipped, absolute grill-trace-layer ban (P4.2's `check_greenfield_assumed` — "no exceptions in this surface") to the FR-row layer, which is stricter than #4's own recorded ceiling ("only where the answer could not be obtained") and contradicts `fr-authoring.md`/`requirement-elicitation.md`/`spec-generation.md`, all of which keep greenfield `assumed` legal when paired with a settlement. Fixed to the form obligation the docs actually state: a bare `assumed` cell is legal when the row carries at least one acceptance criterion (mechanically checkable — presence, not aboutness, which has no deterministic oracle and stays a judgement call); a qualified `assumed: <reason>` cell stays banned unconditionally, since the settlement belongs in an acceptance criterion, not smuggled into the Basis cell. Same mechanism, same tests as #4: `_project_gate_extras.basis_forbids_assumed`, `check_basis_forbids_assumed`. |
