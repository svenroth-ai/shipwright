...
| O38 | the pre-existing race-followup E2E used a non-Git synthetic project whose runtime-created lockfile invalidates the permanent gate | high | **Accepted as fixture repair; production remains strict.** The fixture now mirrors an iterate checkout with a real bare `origin/main`, committed Python-3.11 `uv.lock`, and ignored `.venv`/coverage/run artifacts. Weakening source fingerprinting or treating a missing merge base as `n/a` was rejected because either would recreate false certification. The focused E2E passed 1/1 and the full integration root passed 444/444 (2 deselected). |
| O39 | hashing `uv.lock` can invalidate F0 when `uv run` updates an inconsistent lockfile | med | **Rejected as a relaxation; the invalidation is the contract working.** `uv.lock` controls the dependency graph used by measurement, so coverage gathered before and after its mutation is not one source/config snapshot. The race E2E exposed this exact case and was repaired by creating its Python-3.11 lock before the baseline commit. Excluding the lock or silently accepting an auto-update would certify a dependency state different from the final patch; forcing `--locked` everywhere would also diverge from CI's existing provisioning contract (O32). |
| O40 | publishing the invocation-owned candidate to root `coverage.xml` can overwrite an unrelated concurrent coverage producer | low | **Supported ownership boundary retained.** Root `coverage.xml` is an ignored generated artifact and the established CI/local publication path. The OS lock coordinates all F0 invocations; arbitrary non-F0 tools writing the same conventional output concurrently are outside that lock's guarantee and have normal last-writer semantics. The gate itself never trusts a foreign root report: it measures the UUID-owned candidate first and promotes only after a valid verdict. |
| O41 | `.coverage.f0.lock` might dirty the checkout or enter the private-index patch | low | **Already satisfied empirically.** `git check-ignore -v .coverage.f0.lock` resolves to `.gitignore:49:.coverage*`, so the stable rendezvous is neither shown as untracked nor staged by the private `git add -A`. |
