...
- **Empirical probes run:** focused evidence/F5c tests, an atomic concurrent-writer race, real Git commit/worktree/backfill provenance tests, strict `M`-only worktree-status parsing, CRLF-only checkout normalization with negative type/duplicate-key/formatting regressions, exact-byte Git tests, finalize-bundle integration, CLI surface verification, the repository F0 suite split after a host `KeyboardInterrupt`, test-hygiene scan, Ruff, anti-ratchet, and the CI-parity diff-coverage gate. The final gate measured 425 changed lines at 89% coverage against refreshed `origin/main`, with an unchanged source fingerprint during the final focused run.
