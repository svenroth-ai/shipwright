...
| 6 | glm | high | Content-binding assumes the regenerated manifest's `source_commit` field exists and equals the real commit for a push event (not a merge SHA). | **verified via source inspection (already correct).** Traced the real chain: `ci.yml` calls `ci_manifest_drift_check.py --head-commit "${{ github.sha }}"` → `regenerate_base_head(commit_hash=head_commit)` → `_build(..., source_commit=commit_hash)` → `test_links.build_manifest(..., source_commit=source_commit)`. `github.sha` on a `push` trigger (the ONLY trigger this artifact uploads on, per the `if:` gate) is the literal pushed commit — merge-SHA synthesis is a `pull_request`-trigger-only concern, not applicable here. No code change needed; the concern was already structurally correct. |
