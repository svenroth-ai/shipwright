...
|---|---|---|---|---|---|
| **C1** | audit-run-id-provenance | Detective audit honors Run-ID provenance: B7 matches `Run-ID:`↔`adr_id` (commit-field fallback for legacy); Group E recognizes changelog/release snapshots | monorepo | — | complete (PR #142) |
| **C2** | audit-invocation-resilience | Guarantee PyYAML at audit invocation (PEP-723 deps / `uv run --with` / `--project`) **and** degrade `group_a5` to SKIP (not FAIL) when yaml is genuinely absent | monorepo | — | complete (PR #142) |
| **C3** | fr-linkage-lifecycle | Close the FR-gate bypass on the finalize path (prevent D5-class at write) + accept same-event `new_frs`+`affected_frs` as delivered in D3 | monorepo | — (trace resolved) | in review (PR #147) |
