# Sub-Iterate B1.test — Split test SKILL.md (986 LOC)
...
> for the campaign-level intent and dependency topology.
            ...
            `plugins/shipwright-test/skills/test/SKILL.md` is `986` LOC — well above the 400-LOC
            runtime-prompt budget. Every invocation of `/shipwright-test`
            ...
            - [ ] `plugins/shipwright-test/skills/test/SKILL.md` is <= 300 LOC (preferably ~250).
            - [ ] Each new `references/*.md` is <= 400 LOC.
            - [ ] `shipwright_bloat_baseline.json` entry for
              `plugins/shipwright-test/skills/test/SKILL.md` is REMOVED in the same commit
            ...
            - [ ] All existing `/shipwright-test` invocations behave
              identically (parity probe — see below).
            - [ ] `plugins/shipwright-test/tests/` passes
              unchanged. No test file is touched by this iterate.
            - [ ] Integration tests in `integration-tests/` that
              exercise `/shipwright-test` pass unchanged.

            ## Parity Probe (B1-only — campaign mandate)

            After the split, invoke `/shipwright-test` against a
            ...
            2. Before the split: record the agent's first 30 lines of
               ...
               file under
               `.shipwright/runs/_campaign_B_setup/parity/test-before.txt`.
...
5. **Baseline update is part of the same atomic commit as the
   split** — see CLEANUP-INVARIANT above.
            ...
            Base branch for this sub-iterate: `B1.build`'s post-merge `main` (stacked).
