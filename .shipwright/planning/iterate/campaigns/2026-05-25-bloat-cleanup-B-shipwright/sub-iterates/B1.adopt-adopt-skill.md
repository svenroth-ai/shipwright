# Sub-Iterate B1.adopt — Split adopt SKILL.md (848 LOC)
...
> for the campaign-level intent and dependency topology.
            ...
            `plugins/shipwright-adopt/skills/adopt/SKILL.md` is `848` LOC — well above the 400-LOC
            runtime-prompt budget. Every invocation of `/shipwright-adopt`
            ...
            - [ ] `plugins/shipwright-adopt/skills/adopt/SKILL.md` is <= 300 LOC (preferably ~250).
            - [ ] Each new `references/*.md` is <= 400 LOC.
            - [ ] `shipwright_bloat_baseline.json` entry for
              `plugins/shipwright-adopt/skills/adopt/SKILL.md` is REMOVED in the same commit
            ...
            - [ ] All existing `/shipwright-adopt` invocations behave
              identically (parity probe — see below).
            - [ ] `plugins/shipwright-adopt/tests/` passes
              unchanged. No test file is touched by this iterate.
            - [ ] Integration tests in `integration-tests/` that
              exercise `/shipwright-adopt` pass unchanged.

            ## Parity Probe (B1-only — campaign mandate)

            After the split, invoke `/shipwright-adopt` against a
            ...
            2. Before the split: record the agent's first 30 lines of
               ...
               file under
               `.shipwright/runs/_campaign_B_setup/parity/adopt-before.txt`.
...
5. **Baseline update is part of the same atomic commit as the
   split** — see CLEANUP-INVARIANT above.
            ...
            Base branch for this sub-iterate: `B1.test`'s post-merge `main` (stacked).
