# Sub-Iterate B3 — Split phase_quality.py (1108 LOC)
...
> for the campaign-level intent and dependency topology.
            ...
            `shared/scripts/lib/phase_quality.py` is `1108` LOC — well above the
            300-LOC source budget. The file mixes multiple concerns
            that can be cleanly separated (see split layout below).

            ## Scope

            Split `shared/scripts/lib/phase_quality.py` into the layout below, preserving
            the public import surface so existing callers don't
            break.

            ```
            shared/scripts/lib/phase_quality.py  (DELETE or shim <= 50 LOC)
shared/scripts/lib/phase_quality/
    __init__.py            # re-exports public surface
    project.py             # project phase quality checks
    plan.py                # plan phase quality checks
    build.py               # build phase quality checks
    test.py                # test phase quality checks
    security.py            # security phase quality checks
    deploy.py              # deploy phase quality checks
    iterate.py             # iterate phase quality checks
    _dashboard_render.py   # dashboard rendering (gains bloat column)
            ...
            - [ ] `shared/scripts/lib/phase_quality.py` is either:
            ...
            - [ ] `shipwright_bloat_baseline.json` entry for
              `shared/scripts/lib/phase_quality.py` is REMOVED in the same commit.
            ...
            - [ ] `_dashboard_render.py` renders a bloat-findings
  column on the Compliance Dashboard. The column shows:
  entries-over-limit, in-allowlist, ratchet-delta.
- [ ] `phase_quality/__init__.py` re-exports the existing
  public surface so the Stop hook + audit script keep
  working.
- [ ] All `shared/tests/test_phase_quality*.py` suites
  green (incl. test_phase_quality_rollout,
  test_audit_phase_quality).
- [ ] `/shipwright-compliance` integration probe
  regenerates the dashboard MD and the new bloat column is
  visible + populated against the current baseline.
- [ ] **B3 unblocks B4 + B5** because their tests will
  import from `phase_quality.*` paths. Verify the import
  shape doesn't break either.
...
5. **Baseline update is part of the same atomic commit as the
   split** — see CLEANUP-INVARIANT above.
            ...
            Base branch for this sub-iterate: `B6`'s post-merge `main` (stacked).
