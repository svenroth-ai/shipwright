# Sub-Iterate B4 — Split dev_server.py (997 LOC)
...
> for the campaign-level intent and dependency topology.
            ...
            `shared/scripts/dev_server.py` is `997` LOC — well above the
            300-LOC source budget. The file mixes multiple concerns
            that can be cleanly separated (see split layout below).

            ## Scope

            Split `shared/scripts/dev_server.py` into the layout below, preserving
            the public import surface so existing callers don't
            break.

            ```
            shared/scripts/dev_server.py  (DELETE or shim <= 50 LOC)
shared/scripts/dev_server/
    __init__.py            # re-exports public surface
    __main__.py            # `python -m shared.scripts.dev_server` entry
    spawn.py               # process spawn / detach
    health.py              # health-check polling
    multiservice.py        # multi-service orchestration
            ...
            - [ ] `shared/scripts/dev_server.py` is either:
            ...
            - [ ] `shipwright_bloat_baseline.json` entry for
              `shared/scripts/dev_server.py` is REMOVED in the same commit.
            ...
            - [ ] `python -m shared.scripts.dev_server <args>` works
  identically — verify by running an existing dev-server
  integration probe against a fixture project.
- [ ] `shared/tests/test_dev_server_multiservice.py` green
  against the new layout.
- [ ] The multiservice smoke flow (start two fixture
  services, hit health endpoints, stop) succeeds in
  under the existing timeout budget.
...
5. **Baseline update is part of the same atomic commit as the
   split** — see CLEANUP-INVARIANT above.
            ...
            Base branch for this sub-iterate: `B3`'s post-merge `main` (stacked).
