# Sub-Iterate B1.iterate — Split iterate SKILL.md (1709 LOC)
...
> for the campaign-level intent and dependency topology.
            ...
            `plugins/shipwright-iterate/skills/iterate/SKILL.md` is `1709` LOC — well above the 400-LOC
            runtime-prompt budget. Every invocation of `/shipwright-iterate`
            ...
            - [ ] `plugins/shipwright-iterate/skills/iterate/SKILL.md` is <= 300 LOC (preferably ~250).
            - [ ] Each new `references/*.md` is <= 400 LOC.
            - [ ] `shipwright_bloat_baseline.json` entry for
              `plugins/shipwright-iterate/skills/iterate/SKILL.md` is REMOVED in the same commit
            ...
            - [ ] All existing `/shipwright-iterate` invocations behave
              identically (parity probe — see below).
            - [ ] `plugins/shipwright-iterate/tests/` passes
              unchanged. No test file is touched by this iterate.
            - [ ] Integration tests in `integration-tests/` that
              exercise `/shipwright-iterate` pass unchanged.

            ## Parity Probe (B1-only — campaign mandate)

            After the split, invoke `/shipwright-iterate` against a
            ...
            2. Before the split: record the agent's first 30 lines of
               ...
               file under
               `.shipwright/runs/_campaign_B_setup/parity/iterate-before.txt`.
...
5. **Baseline update is part of the same atomic commit as the
   split** — see CLEANUP-INVARIANT above.
            ...
            Base branch for this sub-iterate: `origin/main` (first unit in the stack).
