# Sub-Iterate B5 — Split orchestrator.py (983 LOC)
...
> for the campaign-level intent and dependency topology.
            ...
            `plugins/shipwright-run/scripts/lib/orchestrator.py` is `983` LOC — well above the
            300-LOC source budget. The file mixes multiple concerns
            that can be cleanly separated (see split layout below).

            ## Scope

            Split `plugins/shipwright-run/scripts/lib/orchestrator.py` into the layout below, preserving
            the public import surface so existing callers don't
            break.

            ```
            plugins/shipwright-run/scripts/lib/orchestrator.py  (DELETE or shim <= 50 LOC)
plugins/shipwright-run/scripts/lib/orchestrator/
    __init__.py            # re-exports public dispatch API
    router.py              # phase dispatch router
    phases/
        __init__.py
        project.py
        design.py
        ...
        iterate.py
            ...
            - [ ] `plugins/shipwright-run/scripts/lib/orchestrator.py` is either:
            ...
            - [ ] `shipwright_bloat_baseline.json` entry for
              `plugins/shipwright-run/scripts/lib/orchestrator.py` is REMOVED in the same commit.
            ...
            - [ ] `orchestrator/router.py` dispatches to each phase
  module with signatures identical to the pre-split
  orchestrator entry points.
- [ ] `plugins/shipwright-run/tests/test_orchestrator.py`
  green against the new layout (the test file is
  grandfathered separately and is not touched).
- [ ] At least one integration test demonstrates
  orchestrator phase routing is unchanged (e.g. by
  dry-running a 2-phase fixture flow and asserting the
  dispatched function names match the pre-split
  recording).
...
5. **Baseline update is part of the same atomic commit as the
   split** — see CLEANUP-INVARIANT above.
            ...
            Base branch for this sub-iterate: `B4`'s post-merge `main` (stacked).
