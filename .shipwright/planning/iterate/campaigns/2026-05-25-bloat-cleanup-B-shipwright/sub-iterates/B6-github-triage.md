# Sub-Iterate B6 — Split github_triage.py (929 LOC)
...
> for the campaign-level intent and dependency topology.
            ...
            `shared/scripts/github_triage.py` is `929` LOC — well above the
            300-LOC source budget. The file mixes multiple concerns
            that can be cleanly separated (see split layout below).

            ## Scope

            Split `shared/scripts/github_triage.py` into the layout below, preserving
            the public import surface so existing callers don't
            break.

            ```
            shared/scripts/github_triage.py  (DELETE or shim <= 50 LOC)
shared/scripts/github_triage/
    __init__.py            # re-exports public surface
    producer.py            # producer side (finding extraction)
    consumer.py            # consumer side (action-unit dispatch)
    state.py               # state read/write
            ...
            - [ ] `shared/scripts/github_triage.py` is either:
            ...
            - [ ] `shipwright_bloat_baseline.json` entry for
              `shared/scripts/github_triage.py` is REMOVED in the same commit.
            ...
            - [ ] `github_triage/__init__.py` re-exports the existing
  public surface (`emit_findings`, `consume_*`, etc.) so
  every current import keeps working.
- [ ] All `shared/tests/test_github_triage*.py` suites
  green (test_github_triage, test_github_triage_action_units,
  test_github_triage_artifact_fallback).
- [ ] `/shipwright-iterate` triage-inbox integration probe
  loads + processes a fixture finding identically.
...
5. **Baseline update is part of the same atomic commit as the
   split** — see CLEANUP-INVARIANT above.
            ...
            Base branch for this sub-iterate: `B2`'s post-merge `main` (stacked).
