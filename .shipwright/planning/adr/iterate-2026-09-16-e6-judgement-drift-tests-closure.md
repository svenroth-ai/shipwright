...
| 6 | OpenAI | high | Redefining the work unit from 19 to 25 without an explicit scope amendment risks ambiguous acceptance. | rejected-with-reason — the campaign orchestrator's own launch prompt for this sub-iterate pre-authorized exactly this ("if you find a different count, stop and treat it as a real finding... write tests for however many you actually find and document the discrepancy in your ADR"); the spec file itself (`e6-judgement-drift-tests.md`, 11 lines) names no such instruction. This is not a silent redefinition; it is an explicit, out-of-band authorization, executed and documented (this ADR, the ledger paragraph, the result.json) — flagged by Stage-1 spec-review (round 1) as a provenance mis-attribution in an earlier revision of this row, which wrongly credited the spec file. |
