...
|---|---|---|---|
| AC01 | **unbound — flagged** | none | Ledger row `C`: "a catalogue of individually deliverable requirements + starting guidance exists". The only seam that checks a NON-EMPTY catalogue (`_validate_project`, `plugins/shipwright-run/scripts/lib/phase_validators.py`) is tested in `plugins/shipwright-run/tests/test_phase_validators_project.py::test_legacy_pre_12_1_gate_still_fires` + `::test_full_canon_project_passes` — a 3rd ADR-044 root beyond this unit's two declared roots. Filed `trg-704cdb22` for campaign-owner ratification (t1/t5 Exception-3 precedent). |
...
| AC15 | **unbound — flagged** | none | The "retired FR number stays permanently taken" clause is only checked by `plugins/shipwright-compliance/tests::test_retired_fr_number_must_not_be_reused` (Group I audit) and `shared/scripts/tests`' `fr_change_history` suite — both outside this unit's two declared roots. Filed with `trg-704cdb22` alongside AC01. The "moved to a retired section" + "coverage stops counting it" clauses ARE provable in `shared/tests/test_drift_parsers.py` (`test_parse_fr_table_excludes_removed_requirements_section` et al., untagged here) but AC15 is conjunctive — a partial binding would misrepresent the whole AC as proven. |
