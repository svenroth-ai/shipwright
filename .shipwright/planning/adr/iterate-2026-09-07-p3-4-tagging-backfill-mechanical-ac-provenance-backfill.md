...
verdict block — ten re-review rounds of the merge-commit diff, the first
after the doubt-review fix commit fell behind `origin/main` and had to be
refreshed via `ensure_current.py`, T2/T3/T4/T6/T7/T8/T9/T10/T11 each fixed
and re-pushed in turn, T5 disputed with a reproduction rather than fixed)
...
| T11 | high (test-coverage) | All three symlink-escape regression tests in this unit (T4's writer test, T4's `apply_writes` unit test, T8's rollback-pair test) used `os.symlink` wrapped in `try/except (OSError, NotImplementedError): pytest.skip(...)` — on Windows this needs `SeCreateSymbolicLinkPrivilege`, which this Windows CI runner (and most contributor machines) lack, so the claimed protection ran UNVERIFIED on the platform it exists for. The repository already has an established, unprivileged alternative (`test_review_scratch.py`'s `_plant_reparse_point` helper: a real directory junction via `mklink /J`, which needs no elevated privilege and exercises the same `IO_REPARSE_TAG_MOUNT_POINT` gap `Path.is_symlink()` misses) that these three tests should have used from the start | accepted-and-fixed — all three tests now junction an ANCESTOR directory (`mklink /J` on Windows, a real `symlink_to` on POSIX) to an external target directory, with the escaping candidate file physically inside it, rather than symlinking the leaf file itself (junctions are directory-only) — this exercises `is_contained`'s resolved-ancestor-path branch rather than its `is_symlink()`-leaf branch, which is in fact the MORE representative Windows attack shape. Verified these three tests now run unconditionally (0 skips, previously 3) on this same Windows dev box where they used to skip |
...
bugs plus one test-coverage gap (cross-FR slug reuse, whole-file regex
over-substitution, per-file mis-attribution inside a group-level-verified
mechanical batch, that same over-substitution mechanism's general form in
both the upgrade AND the insertion write path, a symlink path-traversal
write in the WRITER, an uncaught mid-batch write failure, a
SILENTLY-succeeding mid-batch write failure, that SAME symlink
path-traversal class reopened in `main()`'s own snapshot/restore pair — a
code path the first symlink fix never touched — that same rollback
wrapper's own validation/restore seams not being failure-safe, that SAME
seam's exception clause itself being narrower than the exception it
needed to catch, and all three of this unit's own symlink regression
tests running privilege-gated-skipped rather than unconditionally on the
platform they exist to protect) were found and fixed — two by this
unit's own review cascade before shipping, one (D1) by the orchestrator's
Stage-3 doubt-review after merge-base review, nine (T2, T3, T4, T6, T7,
T8, T9, T10, T11) by the required Tier-3 CI-gate re-review of the merge
commit across ten separate re-review rounds (one of the ten, T5, was a
disputed false positive verified NOT to reproduce and left unfixed) —
plus a metadata bug (epoch-zero timestamp) in
