...
> Auto-generated 2026-09-22T19:48:05.142740Z. Items waiting for triage decision.
> Promote via WebUI Triage tab (when v1b lands) or `shared/scripts/tools/triage_promote.py --id <id> --task-ref EXT:<ref>`.

## Status summary

- Total: 968
- Triage: 11 | Promoted: 8 | Dismissed: 945 | Snoozed: 4
...
<a id="trg-0a3c4edb"></a>
- **22 \[auto\] Give plan\_review the same Codex reasoning-effort contract the other roles have** `id=trg-0a3c4edb | severity=low | kind=improvement → P3/engineering`
...
### Source: iterate-2026-09-21-r1-depends-on-schema (1 item)

<a id="trg-b085c323"></a>
- **23 \[auto\] Promote a unit to merged mid-session so depends\_on gating actually releases dependents** `id=trg-b085c323 | severity=high | kind=improvement → P1/engineering`
  - R1 \(iterate-2026-09-21-r1-depends-on-schema\) added cmd\_next's is\_unit\_ready guard, but the complete-&gt;merged pro…
  - Launch payload (copy into a new Claude session):
    ```text
    C:/Program Files/Git/shipwright-iterate <id>
    ```
  - Promote: `triage_promote.py --id trg-b085c323 --task-ref EXT:<ref>`
...
<a id="trg-5300dc0a"></a>
- **21 \[auto\] Close the 2 open phase-quality Tier-1 FAILs on the iterate phase** `id=trg-5300dc0a | severity=high | kind=bug → P1/engineering`
  - 2 open phase-quality Tier-1 FAIL\(s\) across 1 phase\(s\): iterate.  - iterate:T1 \(T1 every spec FR is mapped in RTM\)…
  - Launch payload (copy into a new Claude session):
    ```text
    /shipwright-compliance
    
    Context: 2 open phase-quality Tier-1 FAIL(s): iterate:T1, iterate:W3.
    Dashboard: .shipwright/compliance/skill-compliance/_dashboard.md
    Each FAIL + remediation is listed in this item's detail.
    ```
  - Promote: `triage_promote.py --id trg-5300dc0a --task-ref EXT:<ref>`
