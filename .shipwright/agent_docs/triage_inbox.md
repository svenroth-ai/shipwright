...
> Auto-generated 2026-08-09T08:08:04.655612Z. Items waiting for triage decision.
> Promote via WebUI Triage tab (when v1b lands) or `shared/scripts/tools/triage_promote.py --id <id> --task-ref EXT:<ref>`.

## Status summary

- Total: 835
- Triage: 24 | Promoted: 4 | Dismissed: 804 | Snoozed: 3
...
<a id="trg-3b72ef37"></a>
- **I2 test-evidence freshness still relies on phase-start mtime** `id=trg-3b72ef37 | severity=medium | kind=compliance → P2/engineering`
  - I2 checks a test-evidence file timestamp against phase\_started. W3 now validates the report against its per-iterate So…
  - Launch payload (copy into a new Claude session):
    ```text
    /shipwright-iterate <id>
    ```
  - Promote: `triage_promote.py --id trg-3b72ef37 --task-ref EXT:<ref>`
