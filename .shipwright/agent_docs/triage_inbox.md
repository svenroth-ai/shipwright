...
> Auto-generated 2026-06-05T20:06:43.582776Z. Items waiting for triage decision.
...
<a id="trg-86d73133"></a>
- **SBOM: 4 undeclared license(s) in plugins/shipwright-plan/pyproject.toml** `id=trg-86d73133 | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    ```
  - Promote: `triage_promote.py --id trg-86d73133 --task-ref EXT:<ref>`

<a id="trg-f70ed149"></a>
- **SBOM: 3 undeclared license(s) in plugins/shipwright-security/pyproject.toml** `id=trg-f70ed149 | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    cd 'plugins/shipwright-security' \
      ...
      && uv run plugins/shipwright-compliance/scripts/tools/update_compliance.py --project-root . --phase iterate
    ```
  - Promote: `triage_promote.py --id trg-f70ed149 --task-ref EXT:<ref>`

<a id="trg-782cb160"></a>
- **SBOM: 2 workspaces missing license metadata for 3 shared package(s)** `id=trg-782cb160 | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    for d in 'plugins/shipwright-adopt' 'plugins/shipwright-compliance' ; do \
      ...
      && uv run plugins/shipwright-compliance/scripts/tools/update_compliance.py --project-root . --phase iterate
    ```
  - Promote: `triage_promote.py --id trg-782cb160 --task-ref EXT:<ref>`

<a id="trg-d3207c91"></a>
- **SBOM: 9 workspaces missing license metadata for 2 shared package(s)** `id=trg-d3207c91 | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    for d in 'plugins/shipwright-build' 'plugins/shipwright-changelog' 'plugins/shipwright-deploy' 'plugins/shipwright-design' 'plugins/shipwright-iterate' 'plugins/shipwright-preview' 'plugins/shipwright-project' 'plugins/shipwright-run' 'plugins/shipwright-test' ; do \
      ...
      && uv run plugins/shipwright-compliance/scripts/tools/update_compliance.py --project-root . --phase iterate
    ```
  - Promote: `triage_promote.py --id trg-d3207c91 --task-ref EXT:<ref>`
