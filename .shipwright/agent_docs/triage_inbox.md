...
> Auto-generated 2026-05-27T20:42:50.226242Z. Items waiting for triage decision.
...
<a id="trg-de1696d9"></a>
- **SBOM: 3 undeclared license(s) in plugins/shipwright-security/pyproject.toml** `id=trg-de1696d9 | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    ```
  - Promote: `triage_promote.py --id trg-de1696d9 --task-ref EXT:<ref>`

<a id="trg-8d9d3f98"></a>
- **SBOM: 4 undeclared license(s) in plugins/shipwright-plan/pyproject.toml** `id=trg-8d9d3f98 | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    cd 'plugins/shipwright-plan' \
      ...
      && uv run plugins/shipwright-compliance/scripts/tools/update_compliance.py --project-root . --phase iterate
    ```
  - Promote: `triage_promote.py --id trg-8d9d3f98 --task-ref EXT:<ref>`

<a id="trg-0656c770"></a>
- **SBOM: 2 workspaces missing license metadata for 3 shared package(s)** `id=trg-0656c770 | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    for d in 'plugins/shipwright-adopt' 'plugins/shipwright-compliance' ; do \
      ...
      && uv run plugins/shipwright-compliance/scripts/tools/update_compliance.py --project-root . --phase iterate
    ```
  - Promote: `triage_promote.py --id trg-0656c770 --task-ref EXT:<ref>`

<a id="trg-456f19f0"></a>
- **SBOM: 10 workspaces missing license metadata for 2 shared package(s)** `id=trg-456f19f0 | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    for d in '.' 'plugins/shipwright-build' 'plugins/shipwright-changelog' 'plugins/shipwright-deploy' 'plugins/shipwright-design' 'plugins/shipwright-iterate' 'plugins/shipwright-preview' 'plugins/shipwright-project' 'plugins/shipwright-run' 'plugins/shipwright-test' ; do \
      ...
      && uv run plugins/shipwright-compliance/scripts/tools/update_compliance.py --project-root . --phase iterate
    ```
  - Promote: `triage_promote.py --id trg-456f19f0 --task-ref EXT:<ref>`
