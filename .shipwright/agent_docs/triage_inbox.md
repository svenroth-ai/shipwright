...
> Auto-generated 2026-06-04T06:14:01.893129Z. Items waiting for triage decision.
...
<a id="trg-130e7bb3"></a>
- **SBOM: 3 undeclared license(s) in plugins/shipwright-security/pyproject.toml** `id=trg-130e7bb3 | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    ```
  - Promote: `triage_promote.py --id trg-130e7bb3 --task-ref EXT:<ref>`

<a id="trg-4c6019fe"></a>
- **SBOM: 4 undeclared license(s) in plugins/shipwright-plan/pyproject.toml** `id=trg-4c6019fe | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    cd 'plugins/shipwright-plan' \
      ...
      && uv run plugins/shipwright-compliance/scripts/tools/update_compliance.py --project-root . --phase iterate
    ```
  - Promote: `triage_promote.py --id trg-4c6019fe --task-ref EXT:<ref>`

<a id="trg-af92774b"></a>
- **SBOM: 9 workspaces missing license metadata for 2 shared package(s)** `id=trg-af92774b | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    for d in 'plugins/shipwright-build' 'plugins/shipwright-changelog' 'plugins/shipwright-deploy' 'plugins/shipwright-design' 'plugins/shipwright-iterate' 'plugins/shipwright-preview' 'plugins/shipwright-project' 'plugins/shipwright-run' 'plugins/shipwright-test' ; do \
      ...
      && uv run plugins/shipwright-compliance/scripts/tools/update_compliance.py --project-root . --phase iterate
    ```
  - Promote: `triage_promote.py --id trg-af92774b --task-ref EXT:<ref>`

<a id="trg-31e24980"></a>
- **SBOM: 2 workspaces missing license metadata for 3 shared package(s)** `id=trg-31e24980 | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    for d in 'plugins/shipwright-adopt' 'plugins/shipwright-compliance' ; do \
      ...
      && uv run plugins/shipwright-compliance/scripts/tools/update_compliance.py --project-root . --phase iterate
    ```
  - Promote: `triage_promote.py --id trg-31e24980 --task-ref EXT:<ref>`
