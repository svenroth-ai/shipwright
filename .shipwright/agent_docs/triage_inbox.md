...
> Auto-generated 2026-05-27T07:46:31.802388Z. Items waiting for triage decision.
...
<a id="trg-ea98bc65"></a>
- **SBOM: 3 undeclared license(s) in plugins/shipwright-security/pyproject.toml** `id=trg-ea98bc65 | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    ```
  - Promote: `triage_promote.py --id trg-ea98bc65 --task-ref EXT:<ref>`

<a id="trg-30a51d3a"></a>
- **SBOM: 4 undeclared license(s) in plugins/shipwright-plan/pyproject.toml** `id=trg-30a51d3a | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    cd 'plugins/shipwright-plan' \
      ...
      && uv run plugins/shipwright-compliance/scripts/tools/update_compliance.py --project-root . --phase iterate
    ```
  - Promote: `triage_promote.py --id trg-30a51d3a --task-ref EXT:<ref>`

<a id="trg-f6c3e863"></a>
- **SBOM: 8 workspaces missing license metadata for 2 shared package(s)** `id=trg-f6c3e863 | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    for d in 'plugins/shipwright-build' 'plugins/shipwright-changelog' 'plugins/shipwright-deploy' 'plugins/shipwright-design' 'plugins/shipwright-preview' 'plugins/shipwright-project' 'plugins/shipwright-run' 'plugins/shipwright-test' ; do \
      ...
      && uv run plugins/shipwright-compliance/scripts/tools/update_compliance.py --project-root . --phase iterate
    ```
  - Promote: `triage_promote.py --id trg-f6c3e863 --task-ref EXT:<ref>`

<a id="trg-31cab473"></a>
- **SBOM: 3 undeclared license(s) in plugins/shipwright-adopt/pyproject.toml** `id=trg-31cab473 | severity=low | kind=compliance → P3/engineering`
  ...
  - Launch payload (copy into a new Claude session):
    ...
    cd 'plugins/shipwright-adopt' \
      ...
      && uv run plugins/shipwright-compliance/scripts/tools/update_compliance.py --project-root . --phase iterate
    ```
  - Promote: `triage_promote.py --id trg-31cab473 --task-ref EXT:<ref>`
