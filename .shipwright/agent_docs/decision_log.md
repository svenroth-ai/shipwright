...
- **REVISED 2026-07-20 by `iterate-2026-07-20-one-header-driven-parser` (campaign requirements-catalog, S4). Rejected option (b) is adopted; the premise that rejected it is falsified.** The premise was "two known formats with stable column orders". There are FIVE (greenfield template, greenfield example, adopt 5-column on disk, adopt 6-column writer, traceability-fixture `FR` header), their orders are not stable, and the prediction that header-detection would be over-engineering was tested by events and lost twice: ADR-048 records a brownfield RTM reporting "Traceability coverage 0%" because this positional regex never matched the 6-column adopt shape, and the S1 golden corpus found the regex rendering the WRONG requirement text in a shipped RTM whenever a row carries more cells than its header declares (FV-3). Rejected option (a) — one implementation — is also adopted: the "drift-protection test guards the duplication until a future iterate consolidates the parser" plan held for 14 months and protected less than it looked. It pinned the two REGEXES only; the removed-section loops wrapped around them were semantic clones nothing checked, and the three header-driven parsers that grew up alongside were never in scope. A drift guard covering 2 of 5 implementations on 1 of 2 halves each read as protection. **Now:** one reader at `shared/scripts/lib/fr_table_reader.py`; all five parsers project from it and none parses. Seven axes on which they silently disagreed are decided once — id strictness (the canonical two-digit form, because manifest schema v3 derives the namespace from the id's group digits), column-map lifetime (set by a header row, survives headings, replaced by the next header), invalid priorities (coerced to Must, not row-dropping), escaped pipes (honoured, closing the round-trip with `markdown_table.escape_cell`), row recognition (leading pipe after stripping; a missing closing pipe does not drop a row), the headerless positional fallback, and the minimum cell count. FV-1, FV-3, FV-4 and FV-5 flip; `golden.json` and `frozen_bugs.py` are updated in the same commit. **Residual, declared:** the strict id rule opens a NEW route to a zero-row parse (a spec using only non-canonical ids), and `check_t1_all_spec_frs_mapped` still cannot distinguish "no spec" from "spec I could not read" — S4 removed FV-1's trigger, not its guard. S5 owns that.
...
- **Consequences:** Operators see a small set of actionable items + ready-to-paste payloads instead of a flood of per-finding mirrors. WebUI is now a thin wrapper over the same lib (CLI tested in CI guarantees the lib is correct). **[RETRACTED 2026-07-27: false — the WebUI is a separate implementation, so the CLI's CI guarantees nothing about it. The two diverge (its reason is optional where the CLI's is required).]** owner_repo unresolved -> emit skipped (no malformed keys). Secret-scanning payload is whitelist-only (no alert content can ever leak into operator-facing surfaces). Pre-iterate legacy items auto-migrate one-shot, per-source-gated, never on a failed fetch.
