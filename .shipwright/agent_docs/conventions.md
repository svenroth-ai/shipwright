...
- **`merge=union` extends past the append-only JSONL logs to the curated agent-docs whose `## …Updates`/`## Learnings` sections are bullet-PREPENDED by parallel iterates: union keeps both prepended bullets (honored server-side too), closing the last cascade piece the regenerate-at-merge resolver can't touch (curated prose, not churn, not in `CHURN_ALLOWLIST`). Keep them a DISTINCT `CURATED_DOC_UNION_PATHS` category so the JSONL `UNION_PATHS`↔churn-allowlist pin + managed-repo signal stay exact; accepted caveat — union is line-based + silent on a same-line non-append edit, so escalate to per-run drop-files if that ever bites.** (iterate-2026-06-12-union-curated-agent-docs)
