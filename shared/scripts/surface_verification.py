...
def main(argv: list[str] | None = None) -> int:
    ...
    # NOTE (iterate-2026-06-13-triage-not-current-work): the F0.5 gate files NO
    # triage item. A fail-closed condition already STOPs the iterate via the
    # exit_code below; mirroring the current run's own blocked work into the
    # "later" backlog was redundant (triage is for deferred follow-ups, the
    # board / events log tracks "now"). The exit codes + evidence block are the
    # gate's contract and are unchanged.
    summary = {
        "exit_code": exit_code,
