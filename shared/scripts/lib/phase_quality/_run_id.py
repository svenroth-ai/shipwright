"""Pointer-reading run-id/worktree-root resolution for the Stop-time audits
(plan § 5.3).

Split out of ``_resolution.py`` (300-LOC ceiling; unrelated concerns already
hosted there — same reasoning as the earlier ``_engagement`` split). The
composite-fallback :func:`resolve_run_id` (steps 1-4 beyond this module's own
pointer read) has since moved on to ``_run_id_resolve`` for the same reason;
``_resolution`` re-exports it from there so existing
...
from ._run_id_wave import (  # noqa: E402
    _in_wave_but_identity_unverifiable,
...
def pointer_run_id(project_root: Path, session_id: str) -> str | None:
    ...
    **Why the liveness check.** Pointers are reaped only by
    ...
    if is_sentinel_run(session_id):
        return None
    if _in_wave_but_identity_unverifiable(project_root):
        return None
...
def pointer_worktree_root(cwd: Path, session_id: str) -> Path | None:
    ...
    if is_sentinel_run(session_id):
        return None
    if _in_wave_but_identity_unverifiable(cwd):
        return None
    try:
...
__all__ = ["pointer_run_id", "pointer_worktree_root"]
