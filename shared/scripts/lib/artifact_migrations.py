...
ARTIFACT_MIGRATIONS: list[dict] = [
    {
        ...
        #   - hyphen-suffixed dir names like ``plugins/shipwright-planning/`` — a
        #     real path, not a legacy reference (``-`` is a legal intra-name char).
        #     ``(?<!\w-)`` is deliberately narrower than adding ``-`` to the class
        #     above: it excludes only a hyphen that CONTINUES a word, so a bare
        #     ``-planning/`` at line start (a diff removal line, an unspaced list
        #     marker) is still caught. Breadth is the axis on which this fix was
        #     rejected three times, so it buys exactly what it must and no more.
        #     It rides the two SEPARATOR patterns only: before a quote a ``-`` says
        #     nothing about the literal's contents.
...
ALLOWLIST: dict[str, list[str]] = {
    ...
    "compliance": [
        ...
        ".shipwright/agent_docs/conventions.md",
        # Regenerated by aggregate_triage.py; renders arbitrary triage prose.
        # Same generated-cache class as the agent_docs siblings below.
        ".shipwright/agent_docs/triage_inbox.md",
        # See "planning" entry for shipwright_test_results.json rationale.
        "shipwright_test_results.json",
        ".shipwright/compliance/change-history.md",
        ...
        # commits on every finalize, so a legacy ref in it cannot be fixed in
        # place and JSON cannot carry the inline marker. Same standing class as
        # change-history.md above. It has no CURRENT finding: its ids were exempt
        # only via the hyphen false positive that
        # iterate-2026-08-01-canon-lookbehind-hyphen removed at the root; the
        # entry is retained for the regeneration property, not for a live hit.
        # (Its sibling regression test needs no entry of its own: it trips
        # nothing under the fixed patterns, and `shared/tests/**` below covers
        # it regardless.)
        ...
        # Templates + Docs (E-scope) — migrated in E. hooks-and-pipeline.md keeps
        # a live basis: the quoted `"compliance"` pipeline-phase literals in its
        # `_LEGACY_PIPELINE_ENTRIES` frozenset and the Plan-v7 quotations.
        # docs/guide.md was exempt only via the hyphen false positive and was
        # removed by iterate-2026-08-01-canon-lookbehind-hyphen.
        ...
        "plugins/shipwright-adopt/scripts/lib/artifact_writer.py",
        # iterate-2026-05-29-fix-path-canon-allowlist. shared/contracts/** is the
        # B8 facade — it names a "compliance" submodule in `__all__`, a quoted
        # literal the hyphen fix does not reach, so this stays on a live hit.
        # shipwright_bloat_baseline.json is a committed JSON register, edited
        # deliberately but unable to carry an inline marker; retained on THAT
        # basis, not a live hit and not the manifest's regeneration one.
        # shared/glossary.md and scripts/install.sh were exempt ONLY via the
        # hyphen false positive and were removed by
        # iterate-2026-08-01-canon-lookbehind-hyphen, which fixed it at the root
        # (pinned per file in test_artifact_path_canon_manifest_allowlist.py).
        "shared/contracts/**",
