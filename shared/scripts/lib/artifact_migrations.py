...
ALLOWLIST: dict[str, list[str]] = {
    ...
    "compliance": [
        ...
        ".shipwright/agent_docs/conventions.md",
        # triage_inbox.md is regenerated by aggregate_triage.py from
        # triage.jsonl and renders SBOM card launchPayloads verbatim — each
        # of which references `plugins/shipwright-compliance/scripts/tools/
        # update_compliance.py`. The trigger is the regex
        # `(?<![\w/.\\])compliance/`: the negative-lookbehind class lacks
        # `-`, so `-compliance/` matches even though `plugins/shipwright-
        # compliance/` is a structurally legitimate plugin source-tree path
        # (NOT a legacy artifact path). Same class as the agent_docs
        # siblings above. Allowlisted in iterate-2026-05-21-empirical-
