...
def generate_handoff(
    ...
    *,
    ...
    lines.extend(render_iterate_progress(project_root, git_info))

    # FR-01.01 (E): the same for a PIPELINE run — which phases are finished and
    # which one was interrupted. Rendered from state the run already holds
    # (`phase_tasks[]` + the loop's dispatch pointer), so it goes ABOVE the
    # event-derived "## Recovery" tally and the legacy checkpoint below it: a
    # reader should meet the authoritative view first. Empty for any config
    # without `phase_tasks[]`, which leaves legacy handoffs unchanged.
    lines.extend(render_pipeline_phases(project_root, run_cfg))
