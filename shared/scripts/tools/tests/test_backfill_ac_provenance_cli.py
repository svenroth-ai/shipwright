...
``test_backfill_ac_provenance_apply_skips.py``, and ``main()``'s
snapshot/restore guarantees (orphan tag, mid-batch write exception, silently
reported write failure) into ``test_backfill_ac_provenance_cli_rollback.py``,
for the same reason.
"""
