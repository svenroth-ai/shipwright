...
guarantees: an orphan tag and an uncaught mid-batch write exception must both
roll back every file the run touched, not just the one that failed. Split
out of ``test_backfill_ac_provenance_cli.py`` at the 300-LOC bloat-baseline
threshold (same precedent as that file's own ``_git.py`` /
``_apply_skips.py`` siblings) -- the pure-logic tests (conflict marking,
apply/validate, status accounting) stay in the sibling file. The
failure-safety tests for ``apply_with_rollback``'s own ``validate_applied``/
``_restore()`` seams and the containment guard live in
``test_backfill_ac_provenance_cli_rollback_failsafe.py``, split out for the
same 300-LOC reason once this file itself started growing past the cap.
"""
