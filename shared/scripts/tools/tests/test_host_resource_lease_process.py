...
run_id, marker, heartbeat, resource = sys.argv[7], sys.argv[8], float(sys.argv[9]), sys.argv[10]
...
def _start(root: Path, lease_root: Path, *, weight: int, capacity: int,
           ...
           heartbeat: float = 30.0, resource: str = "cpu") -> subprocess.Popen:
...
def _wait_queue_entry(lease_root: Path, repo: Path, run_id: str,
    ...
    state_path = lease_root / repository_identity(repo) / "cpu.state.json"
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        try:
...
def test_compatible_weights_overlap_across_sibling_worktrees(tmp_path):
    ...
    p1 = _start(repo, lease_root, weight=2, capacity=4, hold=.8,
                run_id="run-one", marker=m1)
    p2 = None
    try:
        first = _wait_marker(m1)
        p2 = _start(sibling, lease_root, weight=2, capacity=4, hold=.1,
                    run_id="run-two", marker=m2)
        second = _wait_marker(m2)
        assert second["time"] - first["time"] < .7, "compatible leases did not overlap"
    finally:
...
def test_fifo_head_cannot_be_bypassed_by_a_smaller_request(tmp_path):
    ...
    try:
        ...
        first, second = _wait_marker(marks[1]), _wait_marker(marks[2])
