...
**Disclosed limitation (external code review, glm/medium, P3.5 restart round
3 — accepted, NOT fixed here): fingerprint-basis asymmetry with the
automated tool.** ``promote_required_layers.py`` fingerprints the
CI-SOURCED node (``coverage``/``tests`` REPLACED by
``ci_execution_evidence.resolve_execution_evidence``'s confirmed values) —
this CLI fingerprints whatever ``coverage``/``tests`` the WORKING-TREE
manifest happens to carry at ``--manifest`` (default: the on-disk
``.shipwright/compliance/test-traceability.json``, never commit-pinned).
When those two bases genuinely disagree (a stale local file, an
un-regenerated local manifest, or simply a future divergence between local
and CI-observed evidence), ``fingerprint_drifted`` reports drift against a
``demoted`` entry even though the operator's veto responded to the SAME
real-world state the next automated run will also see — re-escalating
``contradicts_recorded_decision`` rather than staying a clean, exitable
skip. Not fixed in this round: retrofitting CI-evidence-awareness into this
CLI (commit-pinning its manifest read, resolving evidence, fingerprinting
the CI-sourced node) is a nontrivial change touching every existing test
here, and rushing it under this round's remaining review budget risked
under-tested new surface more than it closed a currently-narrow gap (an
operator typically runs this shortly after inspecting FRESH local evidence,
so the two bases usually agree in practice). Tracked, not silently
dropped — see the design doc's Known Limitations and
