#!/usr/bin/env python3
"""Send a fixed, non-sensitive DeepSeek request through the production ZDR route."""
...
def _base_evidence(config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    extra_body = deepseek_openrouter_extra_body(config)
    return extra_body, {
        "schema_version": 1,
        "model": resolve_reviewer_model(config, "deepseek", "openrouter"),
...
def run_probe(config: dict[str, Any], *, api_key: str | None) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "schema_version": 1,
            "status": "degraded",
            "reason": "deepseek_routing_policy_invalid",
