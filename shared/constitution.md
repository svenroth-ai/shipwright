...
- File a triage item to track the **current** run's own work — a step you are doing / just did, a gate that already STOPped the run, or a finding a background producer already emits. Triage logs genuine *deferred* follow-ups ("later"); the board / events log owns "now". When in doubt it belongs on the board, not in the backlog
