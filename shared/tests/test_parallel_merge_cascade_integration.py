...
def _ci_sec(critical: int) -> str:
    """Single-line ci-security summary so any two-sided edit textually conflicts,
    exercising the classify → --theirs placeholder leg for the .json snapshot."""
    return f'{{"generated_at": "2026-07-15", "critical": {critical}, "high": 0}}\n'


def test_ci_security_json_conflict_resolves_through_cascade(git_origin_repo, make_worktree, monkeypatch):
    """`.shipwright/compliance/ci-security.json` — the structured CI-security
    summary — is a DERIVED compliance snapshot regenerated by the SAME
    `_update_compliance` producer as the compliance MDs, but a `.json`, so the
    `.md`-shaped `DERIVED_MDS` missed it from `CHURN_ALLOWLIST`. It must resolve
    like the derived MDs, NOT abort the cascade.

    Pre-fix `classify()` marked the `.json` blocking and `integrate` returned
    `{"status": "blocked"}` (hit merging #374). This pins `status == "ok"` across a
    serial drain, as a FOURTH artifact class alongside a regenerated MD, curated
    arch (union) + events (union) — proving the allowlist member composes on real
    git. The regen stub mirrors production: it regenerates the MD but leaves
    `ci-security.json` untouched (``refresh_ci_security`` is fail-soft), so the
    mainline ``--theirs`` placeholder must stand — no conflict markers, no abort.
    """
    work, _origin = git_origin_repo
    _set_repo_identity(work)
    _write(work, ".gitattributes", gu.merge_into(None)[0])
    _write(work, _ARCH, _ARCH_BASE)
    _write(work, _DASH, "base dashboard\n")       # a derived MD → regenerated → follow-up
    _write(work, _CI_SEC, _ci_sec(0))             # the .json summary → fail-soft placeholder stands
    _write(work, "shipwright_events.jsonl", '{"type":"phase_completed","id":"e0"}\n')
    _git(work, "add", "-A")
    _git(work, "commit", "-m", "seed dashboard + ci-security summary")
    _git(work, "push", "origin", "main")

    def _fake_regen(project_root, run_id, **kw):
        # Faithful to production: regenerate the derived MD (run_id-tagged → always a
        # diff → a real follow-up commit), and DO NOT stage/touch ci-security.json —
        # refresh_ci_security is fail-soft (no fresh security.yml run → the summary is
        # left untouched), so the merge's --theirs mainline placeholder stands.
        p = Path(project_root) / _DASH
        p.write_text(f"regenerated from merged tree ({run_id})\n", encoding="utf-8")
        _git(Path(project_root), "add", "--", _DASH)
        return {_DASH: "regenerated"}
    ...
    for n, tag in enumerate(tags, start=1):
        wt = make_worktree(work, f"cisec-{tag}")
        _prepend_arch(wt, f"- **iterate-{tag}** (2026-07-15): {tag} change")
        _write(wt, _DASH, f"{tag} dashboard\n")
        _write(wt, _CI_SEC, _ci_sec(n))  # each edits the summary differently → textual conflict
        _append(wt / "shipwright_events.jsonl",
                f'{{"type":"work_completed","adr_id":"iterate-{tag}","id":"evt-{tag}","commit":""}}')
        _git(wt, "commit", "-am", f"iterate {tag}")
        worktrees.append((tag, wt))
    ...
    _git(work, "fetch", "origin", "main")
    ci_sec = _show(work, "origin/main:" + _CI_SEC)
    assert "<<<<<<<" not in ci_sec
    # Fail-soft common case: no fresh scan → the mainline --theirs placeholder stands.
