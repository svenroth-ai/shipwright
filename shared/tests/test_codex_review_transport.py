...
def test_scrubbed_env_is_a_subset_of_the_allowlist_regardless_of_shape(
    ...
    assert set(env) <= set(transport._ENV_ALLOWLIST)
...
def test_codex_unavailable_returns_error_not_raise(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result == {"status": "error", "transport": "codex", "reason": "not authenticated"}
