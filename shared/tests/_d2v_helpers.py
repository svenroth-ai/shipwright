...
#: Opt-in env var that enables writing the TRACKED evidence artifact. The e2e
#: methods (2–4) are NOT slow-marked, so a default ``pytest shared/tests`` run
#: records them and would otherwise regenerate the tracked file on every run
#: (test-isolation leak). The assertions always run (CI coverage preserved); only
#: the markdown side-effect is gated, so an intentional regen sets this var.
EVIDENCE_WRITE_ENV = "SHIPWRIGHT_D2V_WRITE_EVIDENCE"


def evidence_write_enabled() -> bool:
    """True iff the tracked D2V evidence artifact may be (re)written this run."""
    return os.environ.get(EVIDENCE_WRITE_ENV, "").strip().lower() in (
        "1", "true", "yes", "on",
    )
...
class Evidence:
    ...
    :meth:`flush` writes the markdown once all methods have reported — but ONLY
    when :func:`evidence_write_enabled` (opt-in env var); a default run records +
    asserts without touching the tracked artifact.
    """
    ...
    def flush(self, *, node_ids: list[str]) -> Path:
        # Opt-in gate: writing the TRACKED artifact on every default run is a
        # test-isolation leak (the non-slow e2e methods record results too). The
        # assertions already ran; persisting the markdown is opt-in (regen only).
        if not evidence_write_enabled():
        ...
        lines.append(
            ...
            "`SHIPWRIGHT_D2V_WRITE_EVIDENCE=1 pytest shared/tests/test_d2v_empirical_gate.py "
            "shared/tests/test_d2v_empirical_gate_e2e.py -m slow` (the env var opts "
            "in to writing this tracked artifact; default runs leave it untouched)."
        )
        lines.append("")
