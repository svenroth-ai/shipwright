...
def test_integrate_returns_json_and_aborts_when_merge_commit_blocked(
    ...
    every other failure path. Simulated by forcing the merge ``git commit`` to
    fail (here: a no-op commit on an already-clean index after a churn-only
    reconcile leaves nothing to commit — the canonical 'commit refused' case).
    """
    work, _origin = git_origin_repo
    _set_repo_identity(work)
    _write(work, _DASH, "base dashboard\n")
    ...
    wt = make_worktree(work, "churn-commit-blocked")
    _write(wt, _DASH, "iterate dashboard\n")
    _git(wt, "add", "-A")
    _git(wt, "commit", "-m", "iterate changes dashboard")
    _write(work, _DASH, "main dashboard\n")
    ...
    # patch integrate_main._git to fail specifically on the merge commit call.
    real_git = integrate_main._git

    def failing_git(project_root, *args, **kwargs):
        if args[:1] == ("commit",) and "--no-edit" in args:
            # Mimic a CalledProcessError-raising commit (check=True default).
...
def test_main_cli_reports_merge_commit_failed_status(
    ...
    """The CLI surfaces the new ``merge_commit_failed`` status as JSON + a
    non-zero exit (no traceback)."""
    work, _origin = git_origin_repo
    _set_repo_identity(work)
    _write(work, _DASH, "base\n")
    _git(work, "add", "-A")
    _git(work, "commit", "-m", "seed")
    _git(work, "push", "origin", "main")
    wt = make_worktree(work, "churn-cli-blocked")
    _write(wt, _DASH, "iterate\n")
    _git(wt, "add", "-A")
    _git(wt, "commit", "-m", "iterate")
    _write(work, _DASH, "main\n")
