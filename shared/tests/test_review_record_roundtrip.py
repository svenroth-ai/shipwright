...
def test_the_on_disk_shape_matches_the_pinned_consumer_contract(tmp_path):
    ...
    Both consumer guards MOVED in `shipwright-webui` `ce21323e` (PR #339), and
    this mirror moves with them. Failing either still makes it render every row
    as a data-integrity fault rather than falling back to the markers
    (`review-state.ts`), so the mirror stays literal:

      * the version is now a FLOOR (`>=`), not a strict `!==`. A bump is
        therefore not a break — but it stays at 1 anyway, because a bump buys
        the consumer nothing while `validate_record` rejects a version newer
        than its own constant.
      * a `reviews` key the consumer does not recognise is now RENDERED as an
        extra row, not rejected. Tolerance is additive only: the five it knows
        must still all be PRESENT, or it calls the record unreadable.

    This mirrors the consumer rather than executing it; a cross-repo suite
    ...
    assert record["schema_version"] == 1, "a bump only creates old-cache casualties"
