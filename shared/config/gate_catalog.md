...
| `design.preview-approval` | orchestrator-approve | always | no | Step 3.5: first pixels-seen checkpoint (3 preview screens). SENSITIVE: a human must eyeball the visual direction - never auto-approved. |
...
| `design.review-loop-finalize` | orchestrator-approve | always | no | Step 8.5: all-screens sign-off + phase exit. SENSITIVE: the design phase's human approval gate - subagent runs the automated FR-Coverage self-check, then stops for a human. |
...
| `plan.interview` | auto-default | always | no | The design-decision interview (adaptive 3-5 questions). Largest interactivity surface; answered from context under single_session. **Default:** Answer each design-decision question from the loaded project context (spec, CLAUDE.md, conventions.md, architecture.md) and record it as an assumption in the plan interview record. Human plan-review is deferred to SS3's cross-phase gate-list (2026-07-07). |
...
| `plan.external-review-missing-keys` | auto-default | conditional | no | Step 5 Branch B: no external-review API key. Not silently skippable; the internal review (Step 5-int, always runs first, one extra subagent call) preserves the gate rather than self-assessment. **Default:** Option 2: skip external review, rely on the internal review (opus-plan-reviewer, Step 5-int) which already ran and carries the gate, and log the opt-out reason ('non-interactive single-session run; no OPENROUTER key provisioned'). |
...
| `plan.internal-review-high-severity-declined` | auto-default | conditional | no | A severity:high finding from opus-plan-reviewer was declined or disclosed rather than fixed - either `/shipwright-plan` Step 5-int, or `/shipwright-iterate`'s own internal-review sub-step (`iteration-planning.md` step 0, medium+), which reuses this gate rather than minting an `iterate.*` id. Interactive runs STOP and ask (mirrors Step 5a's reject prompt); single_session auto-answers with a recorded reason. **Default:** Autonomously decline or disclose only with a specific, falsifiable reason tied to project context; if none exists, integrate the finding instead. Surface the finding + reason so it stays auditable even though no human was asked: in `/shipwright-plan`'s plan.md Internal Plan Review section and decision_log.md, or in `/shipwright-iterate`'s iterate spec Internal Plan Review section and iterate ADR when this same gate fires from that consumer's own internal-review sub-step. |
