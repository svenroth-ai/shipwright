...
| `plan.external-review-missing-keys` | auto-default | conditional | no | Step 5 Branch B: no external-review API key. Not silently skippable; the internal Opus review (Step 5-int, always runs first, one extra subagent call) preserves the gate rather than self-assessment. **Default:** Option 2: skip external review, rely on the internal Opus review (opus-plan-reviewer, Step 5-int) which already ran and carries the gate, and log the opt-out reason ('non-interactive single-session run; no OPENROUTER key provisioned'). |
...
| `plan.internal-review-high-severity-declined` | auto-default | conditional | no | Step 5-int: a severity:high finding from opus-plan-reviewer was declined or disclosed rather than fixed. Interactive runs STOP and ask (mirrors Step 5a's reject prompt); single_session auto-answers with a recorded reason. **Default:** Autonomously decline or disclose only with a specific, falsifiable reason tied to project context; if none exists, integrate the finding instead. Surface the finding + reason in the plan.md Internal Plan Review section and decision_log.md so it stays auditable even though no human was asked. |
