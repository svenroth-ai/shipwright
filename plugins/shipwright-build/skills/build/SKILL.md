...
Read `shipwright_run_config.json` to detect **pipeline** vs **standalone** mode and store `invocation_mode` for later steps. See [first-actions](references/first-actions.md) for the full decision tree (incl. the out-of-sequence warning).
