...
Boundaries table (``touches_io_boundary`` → Boundary Probe):
    ...
    build_json_sidecar -> latest.json         -> the report + sidecar readers
    run_scan_and_report-> history/scan-*.json -> the archived sidecar readers
...
# The per-finding triage mirrors are asserted in
# shared/tests/test_security_triage_emit.py: importing the shared `triage` module
# from a plugin pytest session hits the ADR-044 `lib` namespace collision, so the
# producer tests live under shared/tests/.
