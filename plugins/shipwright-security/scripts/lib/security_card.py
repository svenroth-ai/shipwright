...
def build_scan_action_unit(
    *,
    ...
    # Sanitize the manifest HERE rather than trusting the caller to have done it.
    # `coverage` is caller input, and its class labels reach both `detail` and the
    # instruction-bearing payload. The file-reading callers already sanitize, so
    # this is idempotent for them — but a caller passing a raw manifest must not
    # be the difference between a safe card and an injectable one.
