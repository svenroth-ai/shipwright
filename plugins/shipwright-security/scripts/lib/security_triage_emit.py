...
- :func:`emit_findings_to_triage` — one item per finding, the enumeration. It
  carries a stable per-finding dedup key so the same finding on the same line
  is one item across runs. Moved here verbatim from
  ``generate_security_report`` (which is over its bloat baseline) so this file
  can grow the card below without ratcheting it.
- :func:`emit_scan_card` — ONE collapsed action unit per repository carrying
  the per-severity split, what the scan did not check, and the scope question.
  The enumeration says *what* was found; the card says *how much of each* and
  asks how far to go. A card that carries only a total leaves the executing
  agent to decide silently that the low-severity findings do not matter.

Both are best-effort: an emission failure is logged to stderr and swallowed so
it can never block the report the operator actually asked for.
"""
...
def emit_scan_card(
    ...
    *,
    ...
    Returns the new triage id, or ``None`` when there was nothing to act on,
    the appender was unreachable, or the card already exists in the window.
    """
