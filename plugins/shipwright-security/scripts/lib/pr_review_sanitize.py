"""Sanitisation and model-facing metadata for the Tier-3 PR reviewer.

Split out of `pr_review_render` when that file crossed the source-size
guideline (iterate-2026-09-09-pr-review-dict-finding-render) — this half owns
turning an untrusted, PR-controlled path into inert, bounded display data;
`pr_review_render` keeps the comment-assembly logic that consumes it.

Path names handled here come from the PR's own diff, so on an untrusted PR
they are attacker-chosen. `safe_path` is the one chokepoint.
"""
...
__all__ = ["build_pr_meta", "nothing_reviewed_summary", "safe_path", "strip_display_unsafe"]
