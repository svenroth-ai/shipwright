...
boundary (campaign B1.test, 2026-05-25).

After the 986-LOC SKILL.md is split into a thin Kern (<=300 LOC) +
references/Step*.md per-step docs + topical references, two things must
hold structurally:

1. Every `references/...` link inside Kern SKILL.md resolves to an
   existing file on disk.
2. Every `references/Step*.md` (per-step reference) and every topical
...
assert membership for the Step + topical references this iterate
produces. The pre-existing references (`reflection.md`,
`test-layers.md`) are covered by the link-resolution arm.
"""
...
# Per-step references this iterate produces. Names mirror SKILL.md
# section anchors so a future reader can grep both sides.
EXPECTED_STEP_REFERENCES = {
    "step-0-phase-session.md",
    "step-1-unit-tests.md",
    "step-1.5-integration-tests.md",
    "step-1.6-pgtap-tests.md",
    "step-2-smoke-test.md",
    "step-2.5-e2e-spec-generation.md",
    "step-3-playwright-e2e.md",
    "step-3.5-e2e-verification.md",
    "step-3.6-ui-consistency.md",
    "step-3.7-design-fidelity.md",
    "step-3.8-performance-budget.md",
    "step-3.9-stop-dev-server.md",
    "step-3.95-boundary-coverage.md",
    "step-4-security.md",
    "step-5-report-results.md",
}

# Topical references this iterate produces.
EXPECTED_TOPICAL_REFERENCES = {
    "first-actions.md",
    "prerequisite-self-healing.md",
    "results-enforcement.md",
    "completion-gate.md",
...
def test_every_expected_step_reference_exists_and_is_linked() -> None:
    """Each per-step reference file from EXPECTED_STEP_REFERENCES must
...
def test_kern_skill_md_under_300_loc() -> None:
    ...
    cleanup-invariant rule (a)). 986 -> ~250 was the spec target.
    """
