...
Creates `.worktrees/<slug>` off freshly-fetched `origin/<default>` with branch `iterate/<slug>`. Parse the JSON; **`{project_root}` for the rest of the run = the helper's `project_root` field**. `cd` shell into it. Exit codes: `0` ok · `2` slug collision · `3` fetch failed (STOP unless `SHIPWRIGHT_ITERATE_NO_FETCH=1`). One iterate = one worktree = one branch = one PR. `.worktrees/<slug>` is `.gitignore`'d. Re-hydrate `.env*` + `node_modules`/`.venv` per project shape. Cleanup after PR merge: `git worktree remove` + `git branch -D`. **Check `warnings[]` for a `"layer-promotion-sweep CRITICAL"` line before continuing** (exit is still `0` — the sweep never blocks setup, but this one line means an opportunistic FR Layers promotion's guaranteed rollback itself failed, so the promotion commit may still be sitting on THIS branch, reproducing the exact F11 hard-fail hazard `lib.layer_promotion_sweep` exists to prevent): if present, run `git log --oneline -5` on this branch and confirm the TOP commit's subject starts with `chore(compliance): promote` — setup itself skips the canon self-heal/outbox sweep whenever this line fires (round 13), specifically so nothing can land on top of the promotion commit and bury it, but still read the subject rather than assume it. Also run `git status --porcelain` and `git show --stat HEAD` and confirm the worktree is otherwise clean and HEAD touches only the promotion's own files (the ledger plus `spec.md` path(s)) — this is what makes "drop the TOP commit" equivalent to "drop exactly the promotion commit" (round 13). Only once BOTH are confirmed, drop it with `git reset --hard` to its parent (never to a guessed offset, and never when the top commit's subject does NOT match or the worktree is not clean — STOP and investigate instead, since a blind reset here could discard unrelated work). Never assume the next F11 run would have caught this instead, since this iterate may never touch the promoted FR itself.
