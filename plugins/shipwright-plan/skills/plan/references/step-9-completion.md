...
7. **Section Trace Check** — every section names at least one live
   requirement it serves. A section that serves none is work nobody asked for:
   remove it, or find the requirement it belongs to.
8. **Dependency Order** — every dependency a section declares in
   `SECTION_MANIFEST` appears earlier in the manifest than the section naming
   it. See [section-index.md](section-index.md) for the declaration format.
