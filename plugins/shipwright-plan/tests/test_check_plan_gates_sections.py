...
def test_no_planning_decision_logged_fails(tmp_path, planning):
...
def test_findings_count_unmatched_by_logged_entries_fails(tmp_path, planning):
...
def test_findings_count_matched_by_logged_entries_passes(tmp_path, planning):
    (planning / "external_review_state.json").write_text(
        json.dumps({
    ...
    assert run_gates(planning, "sections")[0] == 0


# --- the section gate's E2E-journeys check (Step 9, FR-01.03 #11) ----------


def test_e2e_check_is_skipped_without_a_plugin_root(planning):
    """No plugin root → no config to decide whether E2E is expected at all."""
    assert run_gates(planning, "sections")[0] == 0
