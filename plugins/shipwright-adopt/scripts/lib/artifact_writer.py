...
from lib.render_helpers import _fmt_stack_line, _utc_today, infer_required_layers  # noqa: E402
...
def _render_spec_md(
    *,
    ...
    for f in features:
        ...
        fr_table_rows += f"| {fr_id} | {label} | Must | {desc} | `{source}` | {', '.join(infer_required_layers(f))} (inferred) |\n"
    if not fr_table_rows:
        fr_table_rows = "| FR-01.01 | _no features detected_ | May | Edit manually after adoption | — | unit (inferred) |\n"
    qr_block = ""
