...
Writes `shipwright_known_failures.json` — inherited failures in the shape the
audit phase reads, plus `inherited_coverage_gaps` **beside** them (a missing test
never feeds `baseline_failure_count`, which buys forgiveness for a red run).
Files the follow-up to question the derived catalogue with a person
