...
2. If validation passes, build the commit message via the SSoT helper
   ...
   **Read the counts first.** `unconfirmed_fr_count` comes from
   ...
   catalogue = json.loads(
       (Path(cwd) / ".shipwright/adopt/derived-catalogue.json").read_text("utf-8"))
...
The `Features:` and `Inherited:` values are read from the two artifacts, never
recomputed by hand:

| Banner slot | Source |
|---|---|
| `<N>` / `<U>` | `.shipwright/adopt/derived-catalogue.json` → `total` / `unconfirmed` |
...
never happened is a different fact from a run that found nothing.
