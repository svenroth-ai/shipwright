...
1. **Writes `shipwright_known_failures.json`** — the accepted-baseline register.
   ...
   | Block | Holds | Read by |
   |---|---|---|
   | `known_failures[]` + `baseline_failure_count` | tests that were **already failing** | `shared/scripts/known_failures.load_accepted_baseline` — the ONE reader the audit and the test phase share since #453 |
...
fact from "clean", and writing a confident zero would erase the difference. It is
also a different fact from the shared reader's `present`, which only says a
declaration exists — a register can be present, well-formed, and still describe
...
- Only `test`, `description`, `ticket`, `added`, `count` are copied from each
  failing test. The payload may be assembled from raw test output, and this file
  is committed at Step H — an environment dump or a traceback carrying a home
  path must not ride along.
- **The `command` is evidence, not payload.** It is required (a run that names no
  command was not observed) and then deliberately **dropped**: `baseline_source`
  records the short `source` LABEL instead. A command line is exactly where a
  token or a home path travels (`TOKEN=… pytest`), and whitelisting the failure
  entries while persisting the raw command beside them would leave the boundary
  open at the one field nobody looks at.
