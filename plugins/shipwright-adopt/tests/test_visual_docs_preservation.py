...
def test_existing_design_tokens_md_is_preserved_before_overwrite(tmp_path: Path) -> None:
    ...
    backup = _backup_path(tmp_path, "design_tokens.md.preserved")
...
def test_existing_guideline_md_is_preserved_before_overwrite(tmp_path: Path) -> None:
    ...
    backup = _backup_path(tmp_path, "guideline.md.preserved")
    assert backup.exists()
    assert backup.read_text(encoding="utf-8") == hand_edited
...
def test_preservation_log_records_visual_doc_actions(tmp_path: Path) -> None:
    ...
    assert ".shipwright/agent_docs/design_tokens.md" in files
    assert ".shipwright/agent_docs/guideline.md" in files
...
def test_no_backup_when_visual_docs_absent(tmp_path: Path) -> None:
    ...
    backup_dir = tmp_path / ".shipwright" / "adopt" / "backups" / ".shipwright" / "agent_docs"
...
def test_setup_adopt_lists_visual_docs_in_existing_artifacts(tmp_path: Path) -> None:
    ...
    agent_docs = _agent_docs_root(tmp_path)
    (agent_docs / "design_tokens.md").write_text("x", encoding="utf-8")
    (agent_docs / "guideline.md").write_text("x", encoding="utf-8")

    report = run_preflight(tmp_path, [])
    artifacts = set(report["existing_artifacts"])
    assert ".shipwright/agent_docs/design_tokens.md" in artifacts
    assert ".shipwright/agent_docs/guideline.md" in artifacts
